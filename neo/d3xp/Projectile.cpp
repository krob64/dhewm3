...
void idProjectile::Restore(idRestoreGame* savefile) {
    ...
    if (smokeFly != NULL) {
        ...
        gameLocal.smokeParticles->EmitSmoke(
            smokeFly, gameLocal.time, gameLocal.random.RandomFloat(),
...
void idProjectile::Launch(const idVec3& start,
    ...
    // allow characters to throw projectiles during cinematics, but not the
    // player
    ...
    thruster.SetPosition(&physicsObj, 0,
                         idVec3(GetPhysics()->GetBounds()[0].x, 0, 0));
    ...
    if (*smokeName != '\0') {
        smokeFly = static_cast<const idDeclParticle*>(
...
void idProjectile::Think(void) {
    ...
    if (smokeFly != NULL && smokeFlyTime && !IsHidden()) {
        ...
        if (!gameLocal.smokeParticles->EmitSmoke(
                smokeFly, smokeFlyTime, gameLocal.random.RandomFloat(),
                GetPhysics()->GetOrigin(), dir.ToMat3(), timeGroup /*_D3XP*/)) {
            smokeFlyTime = gameLocal.time;
        }
    }
    ...
    if (renderLight.lightRadius.x > 0.0f && g_projectileLights.GetBool()) {
        renderLight.origin =
            GetPhysics()->GetOrigin() + GetPhysics()->GetAxis() * lightOffset;
        renderLight.axis = GetPhysics()->GetAxis();
        if ((lightDefHandle != -1)) {
            if (lightEndTime > 0 &&
                gameLocal.time <= lightEndTime + gameLocal.GetMSec()) {
                idVec3 color(0, 0, 0);
                if (gameLocal.time < lightEndTime) {
                    float frac = (float)(gameLocal.time - lightStartTime) /
                                 (float)(lightEndTime - lightStartTime);
...
bool idProjectile::Collide(const trace_t& collision, const idVec3& velocity) {
    ...
    if (gameLocal.isClient) {
        if (ClientPredictionCollide(this, spawnArgs, collision, velocity,
    ...
    if ((collision.c.material != NULL) &&
        (collision.c.material->GetSurfaceFlags() & SURF_NOIMPACT)) {
    ...
    // projectiles can apply an additional impulse next to the rigid body
    // physics impulse
    ...
    if (gameLocal.isMultiplayer && ent->IsType(idDoor::Type) &&
        !static_cast<idDoor*>(ent)->IsOpen() &&
        !ent->spawnArgs.GetBool("no_touch")) {
        ent->ProcessEvent(&EV_Activate, this);
    }

    if (ent->IsType(idActor::Type) ||
        (ent->IsType(idAFAttachment::Type) &&
         static_cast<const idAFAttachment*>(ent)->GetBody()->IsType(
             idActor::Type))) {
        ...
        if (!projectileFlags.detonate_on_world) {
            if (!StartSound("snd_ricochet", SND_CHANNEL_ITEM, 0, true, NULL)) {
                ...
                if (len > BOUNCE_SOUND_MIN_VELOCITY) {
                    SetSoundVolume(
                        len > BOUNCE_SOUND_MAX_VELOCITY
                            ? 1.0f
                            : idMath::Sqrt(len - BOUNCE_SOUND_MIN_VELOCITY) *
                                  (1.0f /
                                   idMath::Sqrt(BOUNCE_SOUND_MAX_VELOCITY -
                                                BOUNCE_SOUND_MIN_VELOCITY)));
    ...
    if (ent->fl.takedamage) {
        ...
        if (damageDefName[0] != '\0') {
            ent->Damage(this, owner.GetEntity(), dir, damageDefName,
                        damageScale,
...
void idProjectile::DefaultDamageEffect(idEntity* soundEnt,
    ...
    if (*sound != '\0') {
        soundEnt->StartSoundShader(declManager->FindSound(sound),
                                   SND_CHANNEL_BODY, 0, false, NULL);
    }
    ...
    if (*decal != '\0') {
        gameLocal.ProjectDecal(
            collision.c.point, -collision.c.normal, 8.0f, true,
            projectileDef.GetFloat("decal_size", "6.0"), decal);
    }
}
...
void idProjectile::AddDefaultDamageEffect(const trace_t& collision,
    ...
    if (gameLocal.isServer && fl.networkSync) {
        ...
        msg.WriteInt((collision.c.material != NULL)
                         ? gameLocal.ServerRemapDecl(
                               -1, DECL_MATERIAL, collision.c.material->Index())
                         : -1);
...
void idProjectile::Fizzle(void) {
    ...
    // we need to work out how long the effects last and then remove them at
    // that time for example, bullets have no real effects
...
void idProjectile::Event_RadiusDamage(idEntity* ignore) {
    ...
    if (splash_damage[0] != '\0') {
        gameLocal.RadiusDamage(physicsObj.GetOrigin(), this, owner.GetEntity(),
                               ignore, this, splash_damage, damagePower);
    }
}
...
void idProjectile::Explode(const trace_t& collision, idEntity* ignore) {
    ...
    int surfaceType = collision.c.material != NULL
                          ? collision.c.material->GetSurfaceType()
                          : SURFTYPE_METAL;
    if (!(fxname && *fxname)) {
        if ((surfaceType == SURFTYPE_NONE) || (surfaceType == SURFTYPE_METAL) ||
            (surfaceType == SURFTYPE_STONE)) {
    ...
    int testC = gameLocal.clip.Contents(testOrg, NULL, mat3_identity,
                                        CONTENTS_WATER, this);
    if (testC & CONTENTS_WATER) {
        ...
        splashEnt = static_cast<idFuncEmitter*>(
            gameLocal.SpawnEntityType(idFuncEmitter::Type, &splashArgs));
        ...
        if (!idStr::Cmp(spawnArgs.GetString("def_damage"),
                        "damage_bullet_chaingun")) {
            fxname = NULL;
        }
    }
#endif
    ...
    if (gameLocal.mpGame.IsGametypeFlagBased() &&
        gameLocal.serverInfo.GetBool("si_midnight")) {
    ...
    if (*light_shader) {
        ...
        renderLight.pointLight = true;
        renderLight.lightRadius[0] = renderLight.lightRadius[1] =
            renderLight.lightRadius[2] =
                spawnArgs.GetFloat("explode_light_radius");
        ...
        if (gameLocal.mpGame.IsGametypeFlagBased() &&
            ...
            renderLight.lightRadius[0] = renderLight.lightRadius[1] =
                renderLight.lightRadius[2] =
                    spawnArgs.GetFloat("explode_light_radius") * 2;
        }
        ...
        renderLight.shaderParms[SHADERPARM_TIMEOFFSET] =
            -MS2SEC(gameLocal.time);
...
#endif
            light_fadetime =
                spawnArgs.GetFloat("explode_light_fadetime", "0.5");
    ...
    if (gameLocal.entities[collision.c.entityNum] &&
        spawnArgs.GetBool("bindOnImpact")) {
    ...
    if (fxdebris) {
        const idDict* debris =
            gameLocal.FindEntityDefDict("projectile_debris", false);
        if (debris) {
            ...
            for (int i = 0; i < amount; i++) {
                ...
                debris->Create(owner.GetEntity(), physicsObj.GetOrigin(),
                               dir.ToMat3());
                debris->Launch();
            }
        }
...
================
*/
void idProjectile::Event_CreateProjectile(idEntity* owner,
                                          const idVec3& start,
                                          const idVec3& dir) {
...
================
*/
void idProjectile::Event_LaunchProjectile(const idVec3& start,
                                          const idVec3& dir,
                                          const idVec3& pushVelocity) {
...
void idProjectile::WriteToSnapshot(idBitMsgDelta& msg) const {
    ...
    if (netSyncPhysics) {
        ...
        msg.WriteDeltaFloat(0.0f, velocity[0], RB_VELOCITY_EXPONENT_BITS,
                            RB_VELOCITY_MANTISSA_BITS);
        msg.WriteDeltaFloat(0.0f, velocity[1], RB_VELOCITY_EXPONENT_BITS,
                            RB_VELOCITY_MANTISSA_BITS);
        msg.WriteDeltaFloat(0.0f, velocity[2], RB_VELOCITY_EXPONENT_BITS,
                            RB_VELOCITY_MANTISSA_BITS);
    }
}
...
void idProjectile::ReadFromSnapshot(const idBitMsgDelta& msg) {
    ...
    while (state != newState) {
        switch (state) {
            ...
            case CREATED: {
                // the right origin and direction are required if you want
                // bullet traces
    ...
    if (msg.ReadBits(1)) {
        ...
        velocity.x = msg.ReadDeltaFloat(0.0f, RB_VELOCITY_EXPONENT_BITS,
                                        RB_VELOCITY_MANTISSA_BITS);
        velocity.y = msg.ReadDeltaFloat(0.0f, RB_VELOCITY_EXPONENT_BITS,
                                        RB_VELOCITY_MANTISSA_BITS);
        velocity.z = msg.ReadDeltaFloat(0.0f, RB_VELOCITY_EXPONENT_BITS,
                                        RB_VELOCITY_MANTISSA_BITS);
            ...
            collision.c.material =
                (index != -1)
                    ? static_cast<const idMaterial*>(
                          declManager->DeclByIndex(DECL_MATERIAL, index))
                    : NULL;
...
=================
*/
void idGuidedProjectile::Launch(const idVec3& start,
                                const idVec3& dir,
                                const idVec3& pushVelocity,
                                const float timeSinceFire,
                                const float launchPower,
                                float dmgPower) {
    idProjectile::Launch(start, dir, pushVelocity, timeSinceFire, launchPower,
                         dmgPower);
    if (owner.GetEntity()) {
        if (owner.GetEntity()->IsType(idAI::Type)) {
            ...
            if (enemy.GetEntity() == NULL ||
                !enemy.GetEntity()->IsType(idActor::Type) ||
                (static_cast<idActor*>(enemy.GetEntity())->team ==
                 player->team)) {
...
void idSoulCubeMissile::KillTarget(const idVec3& dir) {
    ...
    if (enemy.GetEntity() && enemy.GetEntity()->IsType(idActor::Type)) {
        ...
        if (*smokeName != '\0') {
            smokeKill = static_cast<const idDeclParticle*>(
                declManager->FindType(DECL_PARTICLE, smokeName));
            smokeKillTime = gameLocal.time;
        }
        ownerEnt = owner.GetEntity();
        if ((act->health > 0) && ownerEnt && ownerEnt->IsType(idPlayer::Type) &&
    ...
    if (state == LAUNCHED) {
        if (killPhase) {
            ...
            if (gameLocal.time < orbitTime + 1500) {
                if (!gameLocal.smokeParticles->EmitSmoke(
                        smokeKill, smokeKillTime,
                        gameLocal.random.CRandomFloat(), orbitOrg,
                        mat3_identity, timeGroup /*_D3XP*/)) {
                    smokeKillTime = gameLocal.time;
                }
            }
        } else {
        ...
        if ((seekPos - physicsObj.GetOrigin()).Length() < 32.0f) {
            if (returnPhase) {
                ...
                if (ownerEnt && ownerEnt->IsType(idPlayer::Type)) {
                    static_cast<idPlayer*>(ownerEnt)->SetSoulCubeProjectile(
                        NULL);
                }
...
void idSoulCubeMissile::GetSeekPos(idVec3& out) {
    if (returnPhase && owner.GetEntity() &&
...
=================
*/
void idSoulCubeMissile::Launch(const idVec3& start,
                               const idVec3& dir,
                               const idVec3& pushVelocity,
                               const float timeSinceFire,
                               const float launchPower,
    ...
    newStart += offs;
    idGuidedProjectile::Launch(newStart, dir, pushVelocity, timeSinceFire,
    ...
    physicsObj.SetClipMask(
        0);  // never collide.. think routine will decide when to detonate
    ...
    physicsObj.SetLinearVelocity(startingVelocity.Length() *
                                 physicsObj.GetAxis()[2]);
    ...
    for (i = 0; i < num; i++) {
        ...
        if (beamTargets[i].modelDefHandle >= 0) {
            beamTargets[i].modelDefHandle =
                gameRenderWorld->AddEntityDef(&beamTargets[i].renderEntity);
        }
    }
...
void idBFGProjectile::Think(void) {
    if (state == LAUNCHED) {
        ...
        for (int i = 0; i < beamTargets.Num(); i++) {
            ...
            idPlayer* player =
                (beamTargets[i].target.GetEntity()->IsType(idPlayer::Type))
                    ? static_cast<idPlayer*>(beamTargets[i].target.GetEntity())
                    : NULL;
#ifdef _D3XP
            ...
            beamEnt = static_cast<idAnimatedEntity*>(
                beamTargets[i].target.GetEntity());
            if (!idStr::Cmp(beamEnt->GetEntityDefName(),
                ...
                beamEnt->GetJointWorldTransform(bodyJoint, gameLocal.time, org,
                                                temp);
            ...
            beamTargets[i].renderEntity.shaderParms[SHADERPARM_BEAM_END_X] =
                org.x;
            beamTargets[i].renderEntity.shaderParms[SHADERPARM_BEAM_END_Y] =
                org.y;
            beamTargets[i].renderEntity.shaderParms[SHADERPARM_BEAM_END_Z] =
                org.z;
            ...
            if (gameLocal.time > nextDamageTime) {
                ...
                if (damageFreq && *(const char*)damageFreq &&
                    beamTargets[i].target.GetEntity() &&
                    (forceDamage ||
                     beamTargets[i].target.GetEntity()->CanDamage(
                         GetPhysics()->GetOrigin(), org))) {
#else
                if (damageFreq && *(const char*)damageFreq &&
                    beamTargets[i].target.GetEntity() &&
                    beamTargets[i].target.GetEntity()->CanDamage(
                    ...
                    beamTargets[i].target.GetEntity()->Damage(
                        this, owner.GetEntity(), org, damageFreq,
                        (damagePower) ? damagePower : 1.0f, INVALID_JOINT);
                } else {
                    beamTargets[i].renderEntity.shaderParms[SHADERPARM_RED] =
                        beamTargets[i]
                            .renderEntity.shaderParms[SHADERPARM_GREEN] =
                            beamTargets[i]
                                ...
                                beamTargets[i]
                                    .renderEntity
                                    .shaderParms[SHADERPARM_ALPHA] = 0.0f;
                    bfgVision = false;
                }
                if (player) {
            ...
            gameRenderWorld->UpdateEntityDef(beamTargets[i].modelDefHandle,
                                             &beamTargets[i].renderEntity);
        }

        if (secondModelDefHandle >= 0) {
            secondModel.origin = GetPhysics()->GetOrigin();
            gameRenderWorld->UpdateEntityDef(secondModelDefHandle,
                                             &secondModel);
        }
...
=================
*/
void idBFGProjectile::Launch(const idVec3& start,
                             const idVec3& dir,
                             const idVec3& pushVelocity,
                             const float timeSinceFire,
                             const float power,
    ...
    // extrapolation_t(EXTRAPOLATION_LINEAR|EXTRAPOLATION_NOSTOP),
    // gameLocal.time, 0, physicsObj.GetAxis().ToAngles(), delta, ang_zero );

    // get all entities touching the bounds
    numListedEntities = gameLocal.clip.EntitiesTouchingBounds(
        bounds, CONTENTS_BODY, entityList, MAX_GENTITIES);
    for (int e = 0; e < numListedEntities; e++) {
        ent = entityList[e];
        assert(ent);

        if (ent == this || ent == owner.GetEntity() || ent->IsHidden() ||
            !ent->IsActive() || !ent->fl.takedamage || ent->health <= 0 ||
        ...
        bt.renderEntity.shaderParms[SHADERPARM_DIVERSITY] =
            gameLocal.random.CRandomFloat() * 0.75;
...
#ifdef _D3XP
    // Major hack for end boss.  :(
    idAnimatedEntity* maledict = static_cast<idAnimatedEntity*>(
        gameLocal.FindEntity("monster_boss_d3xp_maledict_1"));
    ...
    if (maledict) {
        ...
        maledict->GetJointWorldTransform(bodyJoint, gameLocal.time, realPoint,
                                         temp);
...
void idBFGProjectile::Explode(const trace_t& collision, idEntity* ignore) {
    ...
    for (i = 0; i < beamTargets.Num(); i++) {
        ...
        if (!beamTargets[i].target.GetEntity()->CanDamage(
                GetPhysics()->GetOrigin(), dmgPoint)) {
            continue;
        }

        beamTargets[i].renderEntity.shaderParms[SHADERPARM_BEAM_WIDTH] =
            beamWidth;
        ...
        if (damage[0] && (beamTargets[i].target.GetEntity()->entityNumber >
                          gameLocal.numClients - 1)) {
            dir = beamTargets[i].target.GetEntity()->GetPhysics()->GetOrigin() -
                  GetPhysics()->GetOrigin();
            dir.Normalize();
            beamTargets[i].target.GetEntity()->Damage(
                this, ownerEnt, dir, damage, damageScale,
                (collision.c.id < 0)
                    ? CLIPMODEL_ID_TO_JOINT_HANDLE(collision.c.id)
                    : INVALID_JOINT);
        }
...
================
*/
void idDebris::Create(idEntity* owner,
                      const idVec3& start,
                      const idMat3& axis) {
    Unbind();
...
void idDebris::Launch(void) {
    ...
    physicsObj.SetLinearVelocity(axis[0] * velocity[0] + axis[1] * velocity[1] +
                                 axis[2] * velocity[2]);
    physicsObj.SetAngularVelocity(angular_velocity.ToAngularVelocity() * axis);
    ...
    if (*smokeName != '\0') {
        ...
        smokeFlyTime = gameLocal.time;
        gameLocal.smokeParticles->EmitSmoke(
            smokeFly, smokeFlyTime, gameLocal.random.CRandomFloat(),
            GetPhysics()->GetOrigin(), GetPhysics()->GetAxis(),
            timeGroup /*_D3XP*/);
    }
...
================
*/
void idDebris::Killed(idEntity* inflictor,
                      idEntity* attacker,
                      int damage,
                      const idVec3& dir,
                      int location) {
