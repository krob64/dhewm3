...
static const char* moveCommandString[NUM_MOVE_COMMANDS] = {
    ...
    "MOVE_TO_ENEMY",     "MOVE_TO_ENEMYHEIGHT",     "MOVE_TO_ENTITY",
    "MOVE_OUT_OF_RANGE", "MOVE_TO_ATTACK_POSITION", "MOVE_TO_COVER",
    "MOVE_TO_POSITION",  "MOVE_TO_POSITION_DIRECT", "MOVE_SLIDE_TO_POSITION",
    "MOVE_WANDER"};
...
idAASFindCover::idAASFindCover(const idVec3& hideFromPos) {
    int numPVSAreas;
    idBounds bounds(hideFromPos - idVec3(16, 16, 0),
                    hideFromPos + idVec3(16, 16, 64));

    // setup PVS
    numPVSAreas =
        gameLocal.pvs.GetPVSAreas(bounds, PVSAreas, idEntity::MAX_PVS_AREAS);
...
bool idAASFindCover::TestArea(const idAAS* aas, int areaNum) {
    ...
    numPVSAreas = gameLocal.pvs.GetPVSAreas(idBounds(areaCenter).Expand(16.0f),
                                            PVSAreas, idEntity::MAX_PVS_AREAS);
...
============
*/
idAASFindAreaOutOfRange::idAASFindAreaOutOfRange(const idVec3& targetPos,
                                                 float maxDist) {
...
bool idAASFindAreaOutOfRange::TestArea(const idAAS* aas, int areaNum) {
    ...
    gameLocal.clip.TracePoint(trace, targetPos,
...
idAASFindAttackPosition::idAASFindAttackPosition(const idAI* self,
    ...
    excludeBounds =
        idBounds(idVec3(-64.0, -64.0f, -8.0f), idVec3(64.0, 64.0f, 64.0f));
    excludeBounds.TranslateSelf(self->GetPhysics()->GetOrigin());

    // setup PVS
    idBounds bounds(targetPos - idVec3(16, 16, 0),
                    targetPos + idVec3(16, 16, 64));
    numPVSAreas =
        gameLocal.pvs.GetPVSAreas(bounds, PVSAreas, idEntity::MAX_PVS_AREAS);
...
void idAI::Save(idSaveGame* savefile) const {
    ...
    savefile->WriteBool(GetPhysics() ==
                        static_cast<const idPhysics*>(&physicsObj));
}
...
void idAI::Spawn(void) {
    ...
    spawnArgs.GetFloat("projectile_height_to_distance_ratio", "1",
    ...
    while (kv) {
        ...
        if (joint == INVALID_JOINT) {
            gameLocal.Warning("Unknown look_joint '%s' on entity %s",
                              jointName.c_str(), name.c_str());
        } else {
            jointScale = spawnArgs.GetAngles(kv->GetKey(), "0 0 0");
            jointScale.roll = 0.0f;

            // if no scale on any component, then don't bother adding it.  this
            // may be done to zero out rotation from an inherited entitydef.
    ...
    if (head.GetEntity()) {
        ...
        if (*jointname) {
            ...
            if (focusJoint == INVALID_JOINT) {
                gameLocal.Warning("Joint '%s' not found on head on '%s'",
                                  jointname, name.c_str());
            }
        }
    } else {
        jointname = spawnArgs.GetString("bone_focus");
        if (*jointname) {
            ...
            if (focusJoint == INVALID_JOINT) {
                gameLocal.Warning("Joint '%s' not found on '%s'", jointname,
                                  name.c_str());
            }
        }
    }
    ...
    projectileClipModel = NULL;
    idStr projectileName;
    if (spawnArgs.GetString("def_projectile", "", projectileName) &&
        projectileName.Length()) {
        projectileDef = gameLocal.FindEntityDefDict(projectileName);
        CreateProjectile(vec3_origin, viewAxis[0]);
        projectileRadius = projectile.GetEntity()
                               ->GetPhysics()
                               ->GetClipModel()
                               ->GetBounds()
                               .GetRadius();
    ...
    if (num_cinematics || spawnArgs.GetBool("hide") ||
        spawnArgs.GetBool("teleport") || spawnArgs.GetBool("trigger_anim")) {
        fl.takedamage = false;
        physicsObj.SetContents(0);
...
void idAI::Think(void) {
    ...
    if (thinkFlags & TH_THINK) {
        ...
        if (num_cinematics) {
            ...
            // clear the ik before we do anything else so the skeleton doesn't
            // get updated twice
        ...
        // clear pain flag so that we recieve any damage between now and the
        // next time we run the script
        AI_PAIN = false;
        AI_SPECIAL_DAMAGE = 0;
        AI_PUSHED = false;
    ...
    if (fl.hidden && allowHiddenMovement) {
        // UpdateAnimation won't call frame commands when hidden, so call them
        // here when we allow hidden movement
...
void idAI::KickObstacles(const idVec3& dir, float force, idEntity* alwaysKick) {
    ...
    numListedClipModels = gameLocal.clip.ClipModelsTouchingBounds(
        clipBounds, clipmask, clipModelList, MAX_GENTITIES);
    for (i = 0; i < numListedClipModels; i++) {
        ...
        if (obEnt->IsType(idMoveable::Type) &&
            obEnt->GetPhysics()->IsPushable()) {
            ...
            delta.z += 0.5f;
            delta.ToVec2() +=
                perpendicular * gameLocal.random.CRandomFloat() * 0.5f;
            forceVec = delta * force * obEnt->GetPhysics()->GetMass();
            obEnt->ApplyImpulse(this, 0, obEnt->GetPhysics()->GetOrigin(),
                                forceVec);
        }
    }
    ...
    if (alwaysKick) {
        ...
        alwaysKick->ApplyImpulse(this, 0, alwaysKick->GetPhysics()->GetOrigin(),
                                 forceVec);
    }
}
...
void idAI::DrawRoute(void) const {
    if (aas && move.toAreaNum && move.moveCommand != MOVE_NONE &&
        ...
        if (move.moveType == MOVETYPE_FLY) {
            aas->ShowFlyPath(physicsObj.GetOrigin(), move.toAreaNum,
                             move.moveDest);
        } else {
            aas->ShowWalkPath(physicsObj.GetOrigin(), move.toAreaNum,
                              move.moveDest);
        }
    }
}
...
bool idAI::ReachedPos(const idVec3& pos,
                      const moveCommand_t moveCommand) const {
    if (move.moveType == MOVETYPE_SLIDE) {
        ...
        if ((moveCommand == MOVE_TO_ENEMY) || (moveCommand == MOVE_TO_ENTITY)) {
            if (physicsObj.GetAbsBounds().IntersectsBounds(
                    idBounds(pos).Expand(8.0f))) {
                return true;
            }
        } else {
            idBounds bnds(idVec3(-16.0, -16.0f, -8.0f),
                          idVec3(16.0, 16.0f, 64.0f));
            bnds.TranslateSelf(physicsObj.GetOrigin());
            if (bnds.ContainsPoint(pos)) {
                return true;
            }
...
int idAI::PointReachableAreaNum(const idVec3& pos,
    ...
    if (move.moveType == MOVETYPE_FLY) {
        areaNum = aas->PointReachableAreaNum(
            pos, bounds, AREA_REACHABLE_WALK | AREA_REACHABLE_FLY);
    } else {
...
bool idAI::PathToGoal(aasPath_t& path,
    ...
    if (move.moveType == MOVETYPE_FLY) {
        return aas->FlyPathToGoal(path, areaNum, org, goalAreaNum, goal,
                                  travelFlags);
    } else {
        return aas->WalkPathToGoal(path, areaNum, org, goalAreaNum, goal,
                                   travelFlags);
    }
}
...
float idAI::TravelDistance(const idVec3& start, const idVec3& end) const {
    ...
    if (!aas) {
        ...
        if (ai_debugMove.GetBool()) {
            gameRenderWorld->DebugLine(colorBlue, start, end, gameLocal.msec,
                                       false);
            gameRenderWorld->DrawText(
                va("%d", (int)dist), (start + end) * 0.5f, 0.1f, colorWhite,
    ...
    int travelTime;
    if (!aas->RouteToGoalArea(fromArea, start, toArea, travelFlags, travelTime,
                              &reach)) {
        return -1;
    }
...
bool idAI::MoveToEnemyHeight(void) {
    ...
    move.moveDest.z =
        lastVisibleEnemyPos.z + enemyEnt->EyeOffset().z + fly_offset;
    move.goalEntity = enemyEnt;
...
bool idAI::MoveToEnemy(void) {
    ...
    if (ReachedPos(lastVisibleReachableEnemyPos, MOVE_TO_ENEMY)) {
        if (!ReachedPos(lastVisibleEnemyPos, MOVE_TO_ENEMY) ||
            !AI_ENEMY_VISIBLE) {
    ...
    if (aas) {
        ...
        if (!PathToGoal(path, areaNum, physicsObj.GetOrigin(), move.toAreaNum,
                        pos)) {
            AI_DEST_UNREACHABLE = true;
            return false;
        }
...
bool idAI::MoveOutOfRange(idEntity* ent, float range) {
    ...
    if (!aas->FindNearestGoal(goal, areaNum, org, pos, travelFlags, &obstacle,
                              1, findGoal)) {
        StopMove(MOVE_STATUS_DEST_UNREACHABLE);
        AI_DEST_UNREACHABLE = true;
        return false;
...
bool idAI::MoveToAttackPosition(idEntity* ent, int attack_anim) {
    ...
    idAASFindAttackPosition findGoal(this, physicsObj.GetGravityAxis(), ent,
                                     pos, missileLaunchOffset[attack_anim]);
    if (!aas->FindNearestGoal(goal, areaNum, org, pos, travelFlags, &obstacle,
                              1, findGoal)) {
        StopMove(MOVE_STATUS_DEST_UNREACHABLE);
        AI_DEST_UNREACHABLE = true;
...
bool idAI::MoveToCover(idEntity* entity, const idVec3& hideFromPos) {
    ...
    if (!aas->FindNearestGoal(hideGoal, areaNum, org, hideFromPos, travelFlags,
                              &obstacle, 1, findCover)) {
        StopMove(MOVE_STATUS_DEST_UNREACHABLE);
        AI_DEST_UNREACHABLE = true;
        return false;
    }
...
bool idAI::WanderAround(void) {
    ...
    move.moveDest = physicsObj.GetOrigin() +
                    viewAxis[0] * physicsObj.GetGravityAxis() * 256.0f;
...
bool idAI::StepDirection(float dir) {
    ...
    idAI::PredictPath(
        this, aas, org, move.moveDir * 48.0f, 1000, 1000,
        (move.moveType == MOVETYPE_FLY)
            ? SE_BLOCKED
            : (SE_ENTER_OBSTACLE | SE_BLOCKED | SE_ENTER_LEDGE_AREA),
        path);

    if (path.blockingEntity &&
        ((move.moveCommand == MOVE_TO_ENEMY) ||
         (move.moveCommand == MOVE_TO_ENTITY)) &&
        (path.blockingEntity == move.goalEntity.GetEntity())) {
    ...
    if ((move.moveType == MOVETYPE_FLY) && (path.endEvent == SE_BLOCKED)) {
        ...
        idAI::PredictPath(this, aas, org, idVec3(0.0f, 0.0f, -1024.0f), 1000,
                          1000, SE_BLOCKED, path);

        idVec3 floorPos = path.endPos;
        idAI::PredictPath(this, aas, floorPos, move.moveDir * 48.0f, 1000, 1000,
                          SE_BLOCKED, path);
        if (!path.endEvent) {
            move.moveDir.z = -1.0f;
            return true;
        ...
        idAI::PredictPath(this, aas, org, idVec3(0.0f, 0.0f, 256.0f), 1000,
                          1000, SE_BLOCKED, path);
        ...
        for (z = org.z; z <= ceilingPos.z + 64.0f; z += 64.0f) {
            ...
            idAI::PredictPath(this, aas, start, move.moveDir * 48.0f, 1000,
                              1000, SE_BLOCKED, path);
            if (!path.endEvent) {
                move.moveDir.z = 1.0f;
                return true;
...
bool idAI::NewWanderDir(const idVec3& dest) {
    ...
    move.nextWanderTime =
        gameLocal.time + (gameLocal.random.RandomFloat() * 500 + 500);
    ...
    if ((gameLocal.random.RandomInt() & 1) ||
        idMath::Fabs(deltay) > idMath::Fabs(deltax)) {
        tdir = d[1];
        d[1] = d[2];
        d[2] = tdir;
    }
...
bool idAI::GetMovePos(idVec3& seekPos) {
    ...
    if (gameLocal.time > move.blockTime) {
        if (move.moveCommand == MOVE_WANDER) {
            move.moveDest =
                org + viewAxis[0] * physicsObj.GetGravityAxis() * 256.0f;
        } else {
            if (ReachedPos(move.moveDest, move.moveCommand)) {
                StopMove(MOVE_STATUS_DONE);
                seekPos = org;
    ...
    if (!result) {
        // wander around
        if ((gameLocal.time > move.nextWanderTime) ||
            !StepDirection(move.wanderYaw)) {
        ...
        if (ai_debugMove.GetBool()) {
            gameRenderWorld->DebugLine(colorYellow, org, seekPos,
                                       gameLocal.msec, true);
        }
    } else {
        AI_DEST_UNREACHABLE = false;
...
bool idAI::EntityCanSeePos(idActor* actor,
    ...
    handle = gameLocal.pvs.SetupCurrentPVS(actor->GetPVSAreas(),
                                           actor->GetNumPVSAreas());
    ...
    if (results.fraction >= 1.0f ||
        (gameLocal.GetTraceEntity(results) == this)) {
...
void idAI::BlockedFailSafe(void) {
    ...
    if (!physicsObj.OnGround() || enemy.GetEntity() == NULL ||
        (physicsObj.GetOrigin() - move.lastMoveOrigin).LengthSqr() >
            Square(blockedRadius)) {
...
void idAI::Turn(void) {
    ...
    if (anim_turn_angles && animflags.anim_turn) {
        ...
        animator.CurrentAnim(ANIMCHANNEL_LEGS)
            ->SetSyncedAnimWeight(0, 1.0f - frac);
        animator.CurrentAnim(ANIMCHANNEL_LEGS)->SetSyncedAnimWeight(1, frac);
        animator.CurrentAnim(ANIMCHANNEL_TORSO)
            ->SetSyncedAnimWeight(0, 1.0f - frac);
        ...
        current_yaw =
            idMath::AngleNormalize180(anim_turn_yaw + rotateAxis[0].ToYaw());
    } else {
    ...
    if (ai_debugMove.GetBool()) {
        ...
        gameRenderWorld->DebugLine(
            colorRed, org, org + idAngles(0, ideal_yaw, 0).ToForward() * 64,
            gameLocal.msec);
        gameRenderWorld->DebugLine(
            colorGreen, org, org + idAngles(0, current_yaw, 0).ToForward() * 48,
            gameLocal.msec);
        gameRenderWorld->DebugLine(
            colorYellow, org,
            org + idAngles(0, current_yaw + turnVel, 0).ToForward() * 32,
            gameLocal.msec);
    }
}
...
void idAI::ApplyImpulse(idEntity* ent,
    ...
    // FIXME: Jim take a look at this and see if this is a reasonable thing to
    // do instead of a spawnArg flag.. Sabaoth is the only slide monster ( and
    // should be the only one for D3 ) and we don't want him taking physics
    // impulses as it can knock him off the path
...
void idAI::GetMoveDelta(const idMat3& oldaxis,
    ...
    if (modelOffset != vec3_zero) {
        // the pivot of the monster's model is around its origin, and not around
        // the bounding box's origin, so we have to compensate for this when the
        // model is offset so that the monster still appears to rotate around
        // it's origin.
...
void idAI::CheckObstacleAvoidance(const idVec3& goalPos, idVec3& newPos) {
    ...
    obstacle = NULL;
    AI_OBSTACLE_IN_PATH = false;
    foundPath = FindPathAroundObstacles(&physicsObj, aas, enemy.GetEntity(),
                                        origin, goalPos, path);
    if (ai_showObstacleAvoidance.GetBool()) {
        gameRenderWorld->DebugLine(
            colorBlue, goalPos + idVec3(1.0f, 1.0f, 0.0f),
            goalPos + idVec3(1.0f, 1.0f, 64.0f), gameLocal.msec);
        gameRenderWorld->DebugLine(
            foundPath ? colorYellow : colorRed, path.seekPos,
            path.seekPos + idVec3(0.0f, 0.0f, 64.0f), gameLocal.msec);
    }
    ...
    if (!foundPath) {
        ...
        if (path.firstObstacle) {
            ...
            if (physicsObj.GetAbsBounds().Expand(2.0f).IntersectsBounds(
                    path.firstObstacle->GetPhysics()->GetAbsBounds())) {
                obstacle = path.firstObstacle;
            }
        } else if (path.startPosObstacle) {
            AI_OBSTACLE_IN_PATH = true;
            if (physicsObj.GetAbsBounds().Expand(2.0f).IntersectsBounds(
                    path.startPosObstacle->GetPhysics()->GetAbsBounds())) {
...
void idAI::AnimMove(void) {
    ...
    if ((move.moveCommand == MOVE_FACE_ENEMY) && enemy.GetEntity()) {
        TurnToward(lastVisibleEnemyPos);
        goalPos = oldorigin;
    } else if ((move.moveCommand == MOVE_FACE_ENTITY) &&
               move.goalEntity.GetEntity()) {
    ...
    if (move.moveCommand == MOVE_SLIDE_TO_POSITION) {
        if (gameLocal.time < move.startTime + move.duration) {
            goalPos = move.moveDest -
                      move.moveDir * MS2SEC(move.startTime + move.duration -
                                            gameLocal.time);
    ...
    if (ai_debugMove.GetBool()) {
        gameRenderWorld->DebugLine(colorCyan, oldorigin, physicsObj.GetOrigin(),
                                   5000);
    }
    ...
    if (!af_push_moveables && attack.Length() && TestMelee()) {
        ...
        if (blockEnt && blockEnt->IsType(idMoveable::Type) &&
            blockEnt->GetPhysics()->IsPushable()) {
    ...
    if (ai_debugMove.GetBool()) {
        ...
        gameRenderWorld->DebugBounds(colorMagenta, physicsObj.GetBounds(),
                                     move.moveDest, gameLocal.msec);
        gameRenderWorld->DebugLine(
            colorYellow, org + EyeOffset(),
            org + EyeOffset() +
                viewAxis[0] * physicsObj.GetGravityAxis() * 16.0f,
            gameLocal.msec, true);
        DrawRoute();
    }
}
...
void idAI::AdjustFlyingAngles(void) {
    ...
    if (flyTiltJoint != INVALID_JOINT) {
        animator.SetJointAxis(flyTiltJoint, JOINTMOD_WORLD,
...
void idAI::AddFlyBob(idVec3& vel) {
    ...
    if (fly_bob_strength) {
        ...
        fly_bob_add = (viewAxis[1] * idMath::Sin16(t * fly_bob_horz) +
                       viewAxis[2] * idMath::Sin16(t * fly_bob_vert)) *
                      fly_bob_strength;
        vel += fly_bob_add * MS2SEC(gameLocal.msec);
        if (ai_debugMove.GetBool()) {
            const idVec3& origin = physicsObj.GetOrigin();
            gameRenderWorld->DebugArrow(colorOrange, origin,
                                        origin + fly_bob_add, 0);
        }
    }
}
...
void idAI::AdjustFlyHeight(idVec3& vel, const idVec3& goalPos) {
    ...
    if (origin.z > goalPos.z) {
        ...
        idAI::PredictPath(this, aas, goalPos, dest - origin, 1000, 1000,
                          SE_BLOCKED, path);
        ...
        if (ai_debugMove.GetBool()) {
            gameRenderWorld->DebugBounds(goLower ? colorRed : colorGreen,
                                         physicsObj.GetBounds(), path.endPos,
                                         gameLocal.msec);
        }
    }
    ...
    if (!goLower) {
        ...
        if (enemyEnt) {
            end.z = lastVisibleEnemyPos.z + lastVisibleEnemyEyeOffset.z +
                    fly_offset;
        } else {
        ...
        gameLocal.clip.Translation(trace, origin, end,
                                   physicsObj.GetClipModel(), mat3_identity,
                                   MASK_MONSTERSOLID, this);
...
void idAI::FlyMove(void) {
    ...
    if ((move.moveCommand != MOVE_NONE) &&
        ReachedPos(move.moveDest, move.moveCommand)) {
        StopMove(MOVE_STATUS_DONE);
    }
    ...
    if (ai_debugMove.GetBool()) {
        gameLocal.Printf(
            ...
            name.c_str(), moveCommandString[move.moveCommand],
            physicsObj.GetLinearVelocity().Length(), move.speed, fly_speed);
    }
    ...
    if (ai_debugMove.GetBool()) {
        ...
        gameRenderWorld->DebugBounds(colorMagenta, physicsObj.GetBounds(),
                                     move.moveDest, gameLocal.msec);
        gameRenderWorld->DebugLine(colorRed, org,
                                   org + physicsObj.GetLinearVelocity(),
                                   gameLocal.msec, true);
        gameRenderWorld->DebugLine(colorBlue, org, goalPos, gameLocal.msec,
                                   true);
        gameRenderWorld->DebugLine(
            colorYellow, org + EyeOffset(),
            org + EyeOffset() +
...
void idAI::StaticMove(void) {
    ...
    if (ai_debugMove.GetBool()) {
        ...
        gameRenderWorld->DebugBounds(colorMagenta, physicsObj.GetBounds(), org,
                                     gameLocal.msec);
        gameRenderWorld->DebugLine(colorBlue, org, move.moveDest,
                                   gameLocal.msec, true);
        gameRenderWorld->DebugLine(
            colorYellow, org + EyeOffset(),
            org + EyeOffset() +
...
int idAI::ReactionTo(const idEntity* ent) {
    ...
    if (actor->IsType(idPlayer::Type) &&
        static_cast<const idPlayer*>(actor)->noclip) {
...
const idDeclParticle* idAI::SpawnParticlesOnJoint(particleEmitter_t& pe,
    ...
    if (pe.joint == INVALID_JOINT) {
        gameLocal.Warning("Unknown particleJoint '%s' on '%s'", jointName,
                          name.c_str());
        pe.time = 0;
        pe.particle = NULL;
    } else {
        ...
        if (!gameLocal.time) {
            // particles with time of 0 don't show, so set the time differently
            // on the first frame
            pe.time = 1;
        } else {
            pe.time = gameLocal.time;
        }
        pe.particle = static_cast<const idDeclParticle*>(
            declManager->FindType(DECL_PARTICLE, particleName));
        gameLocal.smokeParticles->EmitSmoke(pe.particle, pe.time,
...
void idAI::Killed(idEntity* inflictor,
    ...
    if (spawnArgs.GetString("model_death", "", &modelDeath)) {
        // lost soul is only case that does not use a ragdoll and has a
        // model_death so get the death sound in here
        StartSound("snd_death", SND_CHANNEL_VOICE, 0, false, NULL);
    ...
    if ((attacker && attacker->IsType(idPlayer::Type)) &&
        (inflictor && !inflictor->IsType(idSoulCubeMissile::Type))) {
...
void idAI::PlayCinematic(void) {
    ...
    if (!animname) {
        gameLocal.Warning("missing 'anim%d' key on %s", current_cinematic,
                          name.c_str());
        return;
    }
...
void idAI::Activate(idEntity* activator) {
    ...
    if (num_cinematics) {
        ...
        if (cinematic) {
            ...
            if (head.GetEntity()) {
                // since the body anim was updated, we need to run physics to
...
bool idAI::EnemyPositionValid(void) const {
    ...
    gameLocal.clip.TracePoint(tr, GetEyePosition(),
...
void idAI::SetEnemyPosition(void) {
    ...
    if (move.moveCommand == MOVE_TO_ENEMY) {
        ...
        if (move.moveType == MOVETYPE_FLY) {
            ...
            idAI::PredictPath(this, aas, move.moveDest, end - move.moveDest,
                              1000, 1000, SE_BLOCKED, path);
...
void idAI::UpdateEnemyPosition(void) {
    ...
    if (onGround) {
        // when we don't have an AAS, we can't tell if an enemy is reachable or
        // not, so just assume that he is.
        if (!aas) {
            enemyAreaNum = 0;
    ...
    if (CanSee(enemyEnt, false)) {
        ...
        if (enemyEnt == gameLocal.GetAlertEntity()) {
            float dist =
                (enemyEnt->GetPhysics()->GetOrigin() - org).LengthSqr();
    ...
    if (ai_debugMove.GetBool()) {
        gameRenderWorld->DebugBounds(colorLtGrey,
                                     enemyEnt->GetPhysics()->GetBounds(),
                                     lastReachableEnemyPos, gameLocal.msec);
        gameRenderWorld->DebugBounds(
            colorWhite, enemyEnt->GetPhysics()->GetBounds(),
            lastVisibleReachableEnemyPos, gameLocal.msec);
    }
}
...
idVec3 idAI::FirstVisiblePointOnPath(const idVec3 origin,
    ...
    for (i = 0; i < 10; i++) {
        if (!aas->RouteToGoalArea(curAreaNum, curOrigin, targetAreaNum,
...
void idAI::CalculateAttackOffsets(void) {
    ...
    // issues, just allocate an extra spot for launch offsets so that anim
    // number can be used without subtracting 1.
    missileLaunchOffset.SetGranularity(1);
    ...
    for (i = 1; i <= num; i++) {
        ...
        if (anim) {
            ...
            if (frame >= 0) {
                ...
                if (joint == INVALID_JOINT) {
                    gameLocal.Error(
                        "Invalid joint '%s' on 'launch_missile' frame command "
                        "on frame %d of model '%s'",
                        command->string->c_str(), frame, modelDef->GetName());
                }
                GetJointTransformForAnim(joint, i, FRAME2MS(frame),
                                         missileLaunchOffset[i], axis);
            }
        }
    }
...
bool idAI::GetAimDir(const idVec3& firePos,
    ...
    if (aimAtEnt == enemy.GetEntity()) {
        static_cast<idActor*>(aimAtEnt)->GetAIAimTargets(
            lastVisibleEnemyPos, targetPos1, targetPos2);
    } else if (aimAtEnt->IsType(idActor::Type)) {
        static_cast<idActor*>(aimAtEnt)->GetAIAimTargets(
            aimAtEnt->GetPhysics()->GetOrigin(), targetPos1, targetPos2);
    } else {
    ...
    result = PredictTrajectory(
        firePos, targetPos1, projectileSpeed, projectileGravity,
        projectileClipModel, MASK_SHOT_RENDERMODEL, max_height, ignore,
        aimAtEnt, ai_debugTrajectory.GetBool() ? 1000 : 0, aimDir);
    ...
    max_height = delta.LengthFast() * projectile_height_to_distance_ratio;
    result = PredictTrajectory(
        firePos, targetPos2, projectileSpeed, projectileGravity,
        projectileClipModel, MASK_SHOT_RENDERMODEL, max_height, ignore,
        aimAtEnt, ai_debugTrajectory.GetBool() ? 1000 : 0, aimDir);
...
idProjectile* idAI::LaunchProjectile(const char* jointname,
    ...
    if (!projectileDef) {
        gameLocal.Warning("%s (%s) doesn't have a projectile specified",
                          name.c_str(), GetEntityDefName());
        return NULL;
    }
    ...
    if (((ownerBounds[1][0] - ownerBounds[0][0]) >
         (projBounds[1][0] - projBounds[0][0])) &&
        ((ownerBounds[1][1] - ownerBounds[0][1]) >
         (projBounds[1][1] - projBounds[0][1])) &&
        ((ownerBounds[1][2] - ownerBounds[0][2]) >
         (projBounds[1][2] - projBounds[0][2]))) {
        if ((ownerBounds - projBounds)
                .RayIntersection(muzzle, viewAxis[0], distance)) {
    ...
    gameLocal.clip.Translation(tr, start, muzzle, projClip, axis,
    ...
    if (clampToAttackCone) {
        ...
        // doesn't do things like throw the missile backwards if you're behind
        // him
...
void idAI::DirectDamage(const char* meleeDefName, idEntity* ent) {
    ...
    if (!meleeDef) {
        gameLocal.Error("Unknown damage def '%s' on '%s'", meleeDefName,
                        name.c_str());
    }

    if (!ent->fl.takedamage) {
        const idSoundShader* shader =
            declManager->FindSound(meleeDef->GetString("snd_miss"));
...
bool idAI::TestMelee(void) const {
    ...
    if (ai_debugMove.GetBool()) {
        gameRenderWorld->DebugBounds(colorYellow, bounds, vec3_zero,
                                     gameLocal.msec);
    }
    ...
    if ((trace.fraction == 1.0f) ||
        (gameLocal.GetTraceEntity(trace) == enemyEnt)) {
        return true;
    }
...
bool idAI::AttackMelee(const char* meleeDefName) {
    ...
    if (enemyEnt->IsType(idPlayer::Type) && g_skill.GetInteger() < 2) {
        ...
        player->CalcDamagePoints(this, this, meleeDef, 1.0f, INVALID_JOINT,
                                 &damage, &armor);
    ...
    enemyEnt->Damage(this, this, globalKickDir, meleeDefName, 1.0f,
                     INVALID_JOINT);
...
void idAI::PushWithAF(void) {
    ...
    for (i = 0; i < num; i++) {
        ...
        if (j >= num_pushed) {
            ...
            vel = ent->GetPhysics()->GetAbsBounds().GetCenter() -
                  touchList[i].touchedByBody->GetWorldOrigin();
            vel.Normalize();
            if (attack.Length() && ent->IsType(idActor::Type)) {
                ent->Damage(this, this, vel, attack, 1.0f, INVALID_JOINT);
            } else {
                ent->GetPhysics()->SetLinearVelocity(
                    100.0f * vel, touchList[i].touchedClipModel->GetId());
            }
        }
    }
}
...
void idAI::GetMuzzle(const char* jointname, idVec3& muzzle, idMat3& axis) {
    ...
    if (!jointname || !jointname[0]) {
        muzzle = physicsObj.GetOrigin() +
                 viewAxis[0] * physicsObj.GetGravityAxis() * 14;
        muzzle -=
            physicsObj.GetGravityNormal() * physicsObj.GetBounds()[1].z * 0.5f;
    } else {
        joint = animator.GetJointHandle(jointname);
        if (joint == INVALID_JOINT) {
            gameLocal.Error("Unknown joint '%s' on %s", jointname,
                            GetEntityDefName());
        }
...
void idAI::TriggerWeaponEffects(const idVec3& muzzle) {
    ...
    renderEntity.shaderParms[SHADERPARM_DIVERSITY] =
        gameLocal.random.CRandomFloat();
    ...
    if (flashJointWorld != INVALID_JOINT) {
        ...
        if (worldMuzzleFlash.lightRadius.x > 0.0f) {
            ...
            worldMuzzleFlash.shaderParms[SHADERPARM_TIMEOFFSET] =
                -MS2SEC(gameLocal.time);
            if (worldMuzzleFlashHandle != -1) {
                gameRenderWorld->UpdateLightDef(worldMuzzleFlashHandle,
                                                &worldMuzzleFlash);
            } else {
                worldMuzzleFlashHandle =
                    gameRenderWorld->AddLightDef(&worldMuzzleFlash);
            }
...
void idAI::UpdateMuzzleFlash(void) {
    if (worldMuzzleFlashHandle != -1) {
        if (gameLocal.time >= muzzleFlashEnd) {
            ...
            idVec3 muzzle;
            animator.GetJointTransform(flashJointWorld, gameLocal.time, muzzle,
                                       worldMuzzleFlash.axis);
            animator.GetJointTransform(flashJointWorld, gameLocal.time, muzzle,
                                       worldMuzzleFlash.axis);
            muzzle = physicsObj.GetOrigin() + (muzzle + modelOffset) *
                                                  viewAxis *
...
void idAI::SetChatSound(void) {
    ...
    if (snd && *snd) {
        ...
        chat_time = gameLocal.time + chat_min +
                    gameLocal.random.RandomFloat() * (chat_max - chat_min);
    } else {
        chat_snd = NULL;
    }
}
        ...
        for (int i = 0; i < particles.Num(); i++) {
            if (particles[i].particle && particles[i].time) {
                ...
                if (af.IsActive()) {
                    ...
                    animator.GetJointTransform(particles[i].joint,
                                               gameLocal.time, realVector,
                                               realAxis);
                    realAxis *= renderEntity.axis;
                    realVector = physicsObj.GetOrigin() +
                                 (realVector + modelOffset) *
                                     (viewAxis * physicsObj.GetGravityAxis());
                }

                if (!gameLocal.smokeParticles->EmitSmoke(
                        particles[i].particle, particles[i].time,
                        gameLocal.random.CRandomFloat(), realVector,
                        realAxis)) {
                    if (restartParticles) {
                        particles[i].time = gameLocal.time;
                    } else {
...
bool idAI::UpdateAnimationControllers(void) {
    ...
    if (orientationJoint == INVALID_JOINT) {
        ...
        GetJointWorldTransform(orientationJoint, gameLocal.time,
                               orientationJointPos, orientationJointAxis);
        orientationJointYaw = orientationJointAxis[2].ToYaw();
        orientationJointAxis =
            idAngles(0.0f, orientationJointYaw, 0.0f).ToMat3();
    }

    if (focusJoint != INVALID_JOINT) {
        if (headEnt) {
            headEnt->GetJointWorldTransform(focusJoint, gameLocal.time, eyepos,
                                            axis);
        } else {
        ...
        if (ai_debugMove.GetBool()) {
            gameRenderWorld->DebugLine(colorRed, eyepos,
                                       eyepos + orientationJointAxis[0] * 32.0f,
                                       gameLocal.msec);
        }
    } else {
    ...
    // already up to date because of getting the joints in this function) and
    // then sets their positions, which forces the heirarchy to be updated again
    // next time we get a joint or present the model.  If IK is enabled, or if
    // we have a seperate head, we end up transforming the joints twice per
    // frame.  Characters with no head entity and no ik will only transform
    // their joints once.  Set g_debuganim to the current entity number in order
    // to see how many times an entity transforms the joints per frame.
    ...
    if (!allowJointMod || !allowEyeFocus || (gameLocal.time >= focusTime)) {
        ...
        focusPos = lastVisibleEnemyPos + lastVisibleEnemyEyeOffset -
                   eyeVerticalOffset *
                       enemy.GetEntity()->GetPhysics()->GetGravityNormal();
    } else if (focusEnt->IsType(idActor::Type)) {
        focusPos =
            static_cast<idActor*>(focusEnt)->GetEyePosition() -
            eyeVerticalOffset * focusEnt->GetPhysics()->GetGravityNormal();
    } else {
        focusPos = focusEnt->GetPhysics()->GetOrigin();
    }

    currentFocusPos =
        currentFocusPos + (focusPos - currentFocusPos) * eyeFocusRate;
    ...
    newLookAng.yaw =
        idMath::AngleNormalize180(dir.ToYaw() - orientationJointYaw);
    ...
    if (eyeAng != diff) {
        ...
        if (!angDelta.Compare(ang_zero, 0.1f)) {
            ...
            alignHeadTime =
                gameLocal.time +
                (0.5f + 0.5f * gameLocal.random.RandomFloat()) * focusAlignTime;
        }
    }
    ...
    if ((gameLocal.time >= alignHeadTime) ||
        (gameLocal.time < forceAlignHeadTime)) {
        alignHeadTime =
            gameLocal.time +
            (0.5f + 0.5f * gameLocal.random.RandomFloat()) * focusAlignTime;
    ...
    if (headEnt) {
        ...
        if (allowEyeFocus) {
            ...
            headAnimator->SetJointPos(
                leftEyeJoint, JOINTMOD_WORLD_OVERRIDE,
                eyepos + (axis[0] * 64.0f + left) * headTranspose);
            headAnimator->SetJointPos(
                rightEyeJoint, JOINTMOD_WORLD_OVERRIDE,
                eyepos + (axis[0] * 64.0f - left) * headTranspose);
        } else {
        ...
        if (allowEyeFocus) {
            ...
            animator.SetJointPos(leftEyeJoint, JOINTMOD_WORLD_OVERRIDE,
                                 eyepos + left);
            animator.SetJointPos(rightEyeJoint, JOINTMOD_WORLD_OVERRIDE,
                                 eyepos - left);
        } else {
...
void idCombatNode::DrawDebugInfo(void) {
    ...
    for (ent = gameLocal.spawnedEntities.Next(); ent != NULL;
        ...
        if (node->disabled) {
            color = colorMdGrey;
        } else if (player && node->EntityInView(
                                 player, player->GetPhysics()->GetOrigin())) {
            color = colorYellow;
        } else {
            color = colorRed;
        }
        ...
        if (idMath::Fabs(cone_dot) > 0.1) {
            ...
            gameRenderWorld->DebugLine(color, node->GetPhysics()->GetOrigin(),
                                       (pos1 + pos3) * 0.5f, gameLocal.msec);
...
bool idCombatNode::EntityInView(idActor* actor, const idVec3& pos) {
    ...
    if ((pos.z + bounds[1].z < min_height) ||
        (pos.z + bounds[0].z >= max_height)) {
        return false;
    }
