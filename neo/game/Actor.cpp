...
animFlags_t idAnimState::GetAnimFlags(void) const {
    ...
    if (!disabled && !AnimDone(0)) {
        flags =
            animator->GetAnimFlags(animator->CurrentAnim(channel)->AnimNum());
    }
...
idActor::idActor(void) {
    ...
    scriptThread = NULL;  // initialized by ConstructScriptObject, which is
                          // called by idEntity::Spawn
...
void idActor::Spawn(void) {
    ...
    if (headEnt) {
        ...
        for (kv = spawnArgs.MatchPrefix("copy_joint", NULL); kv != NULL;
            ...
            if (copyJoint.from == INVALID_JOINT) {
                gameLocal.Warning("Unknown copy_joint '%s' on entity %s",
            ...
            if (copyJoint.to == INVALID_JOINT) {
                gameLocal.Warning(
                    "Unknown copy_joint '%s' on head of entity %s",
                    jointName.c_str(), name.c_str());
                continue;
            }
    ...
    if (headAnim) {
        if (headEnt) {
            headAnimator->CycleAnim(ANIMCHANNEL_ALL, headAnim, gameLocal.time,
                                    0);
        } else {
            headAnimator->CycleAnim(ANIMCHANNEL_HEAD, headAnim, gameLocal.time,
                                    0);
        }
    }
    ...
    if (spawnArgs.GetString("sound_bone", "", jointName)) {
        ...
        if (soundJoint == INVALID_JOINT) {
            gameLocal.Warning(
                "idAnimated '%s' at (%s): cannot find joint '%s' for sound "
                "playback",
...
void idActor::SetupHead(void) {
    ...
    if (headModel[0]) {
        ...
        if (joint == INVALID_JOINT) {
            gameLocal.Error("Joint '%s' not found for 'head_joint' on '%s'",
                            jointName.c_str(), name.c_str());
        }
        ...
        headEnt = static_cast<idAFAttachment*>(
            gameLocal.SpawnEntityType(idAFAttachment::Type, &args));
        ...
        origin =
            renderEntity.origin + (origin + modelOffset) * renderEntity.axis;
        attach.ent = headEnt;
...
void idActor::CopyJointsFromBodyToHead(void) {
    ...
    for (i = 0; i < copyJoints.Num(); i++) {
        if (copyJoints[i].mod == JOINTMOD_WORLD_OVERRIDE) {
            ...
            GetJointWorldTransform(copyJoints[i].from, gameLocal.time, pos,
                                   axis);
            pos -= headEnt->GetPhysics()->GetOrigin();
            headAnimator->SetJointPos(copyJoints[i].to, copyJoints[i].mod,
                                      pos * mat);
            headAnimator->SetJointAxis(copyJoints[i].to, copyJoints[i].mod,
                                       axis * mat);
        } else {
            animator.GetJointLocalTransform(copyJoints[i].from, gameLocal.time,
                                            pos, axis);
...
void idActor::Save(idSaveGame* savefile) const {
    ...
    if (idealState) {
        idLexer src(idealState->Name(), idStr::Length(idealState->Name()),
                    "idAI::Save");
    ...
    if (headEnt) {
        ...
        if (!spawnArgs.GetFloat("eye_height", "0", eyeOffset.z)) {
            ...
            if (anim && (leftEyeJoint != INVALID_JOINT)) {
                idVec3 pos;
                idMat3 axis;
                headEnt->GetAnimator()->PlayAnim(ANIMCHANNEL_ALL, anim,
                                                 gameLocal.time, 0);
                headEnt->GetAnimator()->GetJointTransform(
                    leftEyeJoint, gameLocal.time, pos, axis);
        ...
        if (!spawnArgs.GetFloat("eye_height", "0", eyeOffset.z)) {
            ...
            if (anim && (leftEyeJoint != INVALID_JOINT)) {
                ...
                animator.GetJointTransform(leftEyeJoint, gameLocal.time, pos,
                                           axis);
                animator.ClearAllAnims(gameLocal.time, 0);
...
void idActor::CheckBlink(void) {
    ...
    if (!blink_anim || (health <= 0) || !allowEyeFocus ||
        (blink_time > gameLocal.time)) {
        return;
    }

    idEntity* headEnt = head.GetEntity();
    if (headEnt) {
        headEnt->GetAnimator()->PlayAnim(ANIMCHANNEL_EYELIDS, blink_anim,
                                         gameLocal.time, 1);
    } else {
    ...
    blink_time = gameLocal.time + blink_min +
                 gameLocal.random.RandomFloat() * (blink_max - blink_min);
}
...
idThread* idActor::ConstructScriptObject(void) {
    ...
    if (!constructor) {
        gameLocal.Error("Missing constructor on '%s' for entity '%s'",
                        scriptObject.GetTypeName(), name.c_str());
    }
...
const function_t* idActor::GetScriptFunction(const char* funcname) {
    ...
    if (!func) {
        scriptThread->Error("Unknown function '%s' in '%s'", funcname,
...
void idActor::UpdateScript(void) {
    ...
    if (i == 20) {
        scriptThread->Warning(
            "idActor::UpdateScript: exited loop to prevent lockup");
    }
}
...
idVec3 idActor::GetEyePosition(void) const {
    return GetPhysics()->GetOrigin() +
           (GetPhysics()->GetGravityNormal() * -eyeOffset.z);
}
...
void idActor::GetAIAimTargets(const idVec3& lastSightPos,
    ...
    chestPos =
        (headPos + lastSightPos + GetPhysics()->GetBounds().GetCenter()) * 0.5f;
}
...
void idActor::LinkCombat(void) {
    ...
    if (combatModel) {
        combatModel->Link(gameLocal.clip, this, 0, renderEntity.origin,
                          renderEntity.axis, modelDefHandle);
    }
    headEnt = head.GetEntity();
    if (headEnt) {
        headEnt->LinkCombat();
...
bool idActor::StartRagdoll(void) {
    ...
    slomoStart = MS2SEC(gameLocal.time) +
                 spawnArgs.GetFloat("ragdoll_slomoStart", "-1.6");
    slomoEnd =
        MS2SEC(gameLocal.time) + spawnArgs.GetFloat("ragdoll_slomoEnd", "0.8");
    ...
    jointFrictionDentStart =
        MS2SEC(gameLocal.time) +
        spawnArgs.GetFloat("ragdoll_jointFrictionStart", "0.2");
    jointFrictionDentEnd =
        MS2SEC(gameLocal.time) +
        spawnArgs.GetFloat("ragdoll_jointFrictionEnd", "1.2");

    // set joint friction dent
    af.GetPhysics()->SetJointFrictionDent(
        jointFrictionDent, jointFrictionDentStart, jointFrictionDentEnd);

    contactFrictionDent =
        spawnArgs.GetFloat("ragdoll_contactFrictionDent", "0.1");
    contactFrictionDentStart =
        MS2SEC(gameLocal.time) +
        spawnArgs.GetFloat("ragdoll_contactFrictionStart", "1.0");
    contactFrictionDentEnd =
        MS2SEC(gameLocal.time) +
        spawnArgs.GetFloat("ragdoll_contactFrictionEnd", "2.0");

    // set contact friction dent
    af.GetPhysics()->SetContactFrictionDent(
        contactFrictionDent, contactFrictionDentStart, contactFrictionDentEnd);
...
void idActor::Attach(idEntity* ent) {
    ...
    joint = animator.GetJointHandle(jointName);
    if (joint == INVALID_JOINT) {
        gameLocal.Error("Joint '%s' not found for attaching '%s' on '%s'",
                        jointName.c_str(), ent->GetClassname(), name.c_str());
    }
    ...
    if (channel == ANIMCHANNEL_HEAD) {
        ...
        if (headEnt) {
            ...
            if (syncAnim) {
                ...
                if (anim) {
                    cycle =
                        animator.CurrentAnim(syncToChannel)->GetCycleCount();
                    starttime =
                        animator.CurrentAnim(syncToChannel)->GetStartTime();
                    headAnimator->PlayAnim(ANIMCHANNEL_ALL, anim,
                                           gameLocal.time, blendTime);
                    headAnimator->CurrentAnim(ANIMCHANNEL_ALL)
                        ->SetCycleCount(cycle);
                    headAnimator->CurrentAnim(ANIMCHANNEL_ALL)
        ...
        if (headEnt) {
            ...
            if (syncAnim) {
                ...
                if (anim) {
                    cycle = headAnimator->CurrentAnim(ANIMCHANNEL_ALL)
                                ->GetCycleCount();
                    starttime = headAnimator->CurrentAnim(ANIMCHANNEL_ALL)
                                    ->GetStartTime();
        ...
        animator.SyncAnimChannels(channel, syncToChannel, gameLocal.time,
                                  blendTime);
    }
}
...
bool idActor::Pain(idEntity* inflictor,
    ...
    if (animPrefix.Length()) {
        if (damageGroup.Length() && (damageGroup != "legs")) {
            sprintf(painAnim, "%s_pain_%s", animPrefix.c_str(),
                    damageGroup.c_str());
...
void idActor::PlayFootStepSound(void) {
    ...
    if (material != NULL) {
        sound = spawnArgs.GetString(
            va("snd_footstep_%s",
               gameLocal.sufaceTypeNames[material->GetSurfaceType()]));
    }
    if (*sound == '\0') {
...
void idActor::Event_DisableEyeFocus(void) {
    ...
    idEntity* headEnt = head.GetEntity();
    if (headEnt) {
        headEnt->GetAnimator()->Clear(ANIMCHANNEL_EYELIDS, gameLocal.time,
                                      FRAME2MS(2));
    } else {
        ...
        case ANIMCHANNEL_HEAD:
            ...
            if (headEnt) {
                ...
                if (!flags.prevent_idle_override) {
                    if (torsoAnim.IsIdle()) {
                        torsoAnim.animBlendFrames =
                            headAnim.lastAnimBlendFrames;
                        SyncAnimChannels(ANIMCHANNEL_TORSO, ANIMCHANNEL_HEAD,
                                         headAnim.lastAnimBlendFrames);
                        if (legsAnim.IsIdle()) {
                            legsAnim.animBlendFrames =
                                headAnim.lastAnimBlendFrames;
                            SyncAnimChannels(ANIMCHANNEL_LEGS, ANIMCHANNEL_HEAD,
                                             headAnim.lastAnimBlendFrames);
                        }
                    }
                }
        ...
        case ANIMCHANNEL_TORSO:
            ...
            if (!flags.prevent_idle_override) {
                if (headAnim.IsIdle()) {
                    ...
                    SyncAnimChannels(ANIMCHANNEL_HEAD, ANIMCHANNEL_TORSO,
                                     torsoAnim.lastAnimBlendFrames);
                }
                if (legsAnim.IsIdle()) {
                    legsAnim.animBlendFrames = torsoAnim.lastAnimBlendFrames;
                    SyncAnimChannels(ANIMCHANNEL_LEGS, ANIMCHANNEL_TORSO,
                                     torsoAnim.lastAnimBlendFrames);
                }
            }
            break;
            ...
            if (!flags.prevent_idle_override) {
                if (torsoAnim.IsIdle()) {
                    ...
                    if (headAnim.IsIdle()) {
                        ...
                        SyncAnimChannels(ANIMCHANNEL_HEAD, ANIMCHANNEL_LEGS,
                                         legsAnim.lastAnimBlendFrames);
                    }
                }
            }
    ...
    switch (channel) {
        case ANIMCHANNEL_HEAD:
            ...
            if (torsoAnim.GetAnimFlags().prevent_idle_override) {
                ...
                // everything is idle, so play the anim on the head and copy it
...
void idActor::Event_SetSyncedAnimWeight(int channel, int anim, float weight) {
    ...
    switch (channel) {
        case ANIMCHANNEL_HEAD:
            if (headEnt) {
                animator.CurrentAnim(ANIMCHANNEL_ALL)
                    ->SetSyncedAnimWeight(anim, weight);
            } else {
                animator.CurrentAnim(ANIMCHANNEL_HEAD)
                    ->SetSyncedAnimWeight(anim, weight);
            }
            if (torsoAnim.IsIdle()) {
...
void idActor::Event_CheckAnim(int channel, const char* animname) {
    if (!GetAnim(channel, animname)) {
        if (animPrefix.Length()) {
            gameLocal.Error("Can't find anim '%s_%s' for '%s'",
                            animPrefix.c_str(), animname, name.c_str());
        } else {
            gameLocal.Error("Can't find anim '%s' for '%s'", animname,
                            name.c_str());
        }
    }
}
        ...
        if (channel == ANIMCHANNEL_HEAD) {
            if (head.GetEntity()) {
                idThread::ReturnString(
                    head.GetEntity()->GetAnimator()->AnimFullName(anim));
                return;
            }
        } else {
        ...
        if (channel == ANIMCHANNEL_HEAD) {
            if (head.GetEntity()) {
                idThread::ReturnFloat(
                    MS2SEC(head.GetEntity()->GetAnimator()->AnimLength(anim)));
                return;
            }
        } else {
...
void idActor::Event_AnimDistance(int channel, const char* animname) {
    ...
    if (anim) {
        if (channel == ANIMCHANNEL_HEAD) {
            if (head.GetEntity()) {
                idThread::ReturnFloat(head.GetEntity()
                                          ->GetAnimator()
                                          ->TotalMovementDelta(anim)
                                          .Length());
                return;
            }
        } else {
...
void idActor::Event_NextEnemy(idEntity* ent) {
    ...
    if (!ent || (ent == this)) {
        ...
        if (actor->enemyNode.ListHead() != &enemyList) {
            gameLocal.Error("'%s' is not in '%s' enemy list",
                            actor->name.c_str(), name.c_str());
        }
    }
