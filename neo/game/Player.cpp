...
const int ASYNC_PLAYER_INV_AMMO_BITS =
    idMath::BitsForInteger(999);  // 9 bits to cover the range [0, 999]
...
idVec3 idPlayer::colorBarTable[5] = {
    idVec3(0.25f, 0.25f, 0.25f), idVec3(1.00f, 0.00f, 0.00f),
    idVec3(0.00f, 0.80f, 0.10f), idVec3(0.20f, 0.50f, 0.80f),
...
void idInventory::RestoreInventory(idPlayer* owner, const idDict& dict) {
    ...
    if (g_skill.GetInteger() >= 3) {
        Give(owner, dict, "weapon", dict.GetString("weapon_nightmare"), NULL,
             false);
    } else {
...
bool idInventory::Give(idPlayer* owner,
    ...
    if (!idStr::Icmpn(statname, "ammo_", 5)) {
        ...
        for (pos = value; pos != NULL; pos = end) {
            ...
            // not for D3 SP .. there is only one case in the game where you can
            // get a no ammo weapon when you might already have it, in that case
            // it is more conistent to pick it up
            ...
            if (!gameLocal.world->spawnArgs.GetBool("no_Weapons") ||
                ...
                if ((weapons & (1 << i)) == 0 || gameLocal.isMultiplayer) {
                    ...
                    if (owner->hud && updateHud &&
                        lastGiveTime + 1000 < gameLocal.time) {
...
void idInventory::Drop(const idDict& spawnArgs,
    ...
    // also remove the ammo associated with the weapon as we pushed it in the
    // item
...
bool idInventory::UseAmmo(ammo_t type, int amount) {
    ...
    if (ammo[type] >= 0) {
        ammo[type] -= amount;
        ammoPredictTime =
            gameLocal.time;  // mp client: we predict this. mark time so we're
                             // not confused by snapshots
    }
...
void idPlayer::Spawn(void) {
    ...
    if (entityNumber >= MAX_CLIENTS) {
        gameLocal.Error(
            "entityNum > MAX_CLIENTS for player.  Player may only be spawned "
            "with a client.");
    }
    ...
    if (!gameLocal.isMultiplayer || entityNumber == gameLocal.localClientNum) {
        ...
        if (spawnArgs.GetString("cursor", "", temp)) {
            cursor = uiManager->FindGui(temp, true, gameLocal.isMultiplayer,
    ...
    // supress model in non-player views, but allow it in mirrors and remote
    // views
    ...
    if (gameLocal.isMultiplayer) {
        ...
        if (!gameLocal.isClient) {
            // set yourself ready to spawn. idMultiplayerGame will decide
            // when/if appropriate and call SpawnFromSpawnSpot
            SetupWeaponEntity();
            SpawnFromSpawnSpot();
            forceRespawn = true;
            assert(spectating);
    ...
    // trigger playtesting item gives, if we didn't get here from a previous
    // level the devmap key will be set on the first devmap, but cleared on any
    // level transitions
    ...
    if (hud) {
        // We can spawn with a full soul cube, so we need to make sure the hud
        // knows this
        if (weapon_soulcube > 0 &&
            (inventory.weapons & (1 << weapon_soulcube))) {
            int max_souls = inventory.MaxAmmoForAmmoClass(this, "ammo_souls");
            if (inventory.ammo[idWeapon::GetAmmoNumForName("ammo_souls")] >=
                max_souls) {
    ...
    if (GetPDA()) {
        ...
        GetPDA()->SetSecurity(
            common->GetLanguageDict()->GetString("#str_00066"));
    }
    ...
    if (!gameLocal.isMultiplayer) {
        if (g_skill.GetInteger() < 2) {
            ...
            g_damageScale.SetFloat(1.0f);
            g_armorProtection.SetFloat((g_skill.GetInteger() < 2) ? 0.4f
                                                                  : 0.2f);
...
void idPlayer::Save(idSaveGame* savefile) const {
    ...
    // don't save spawnAnglesSet, since we'll have to reset them after loading
    // the savegame
    ...
    if (hud) {
        hud->SetStateString("message",
                            common->GetLanguageDict()->GetString("#str_02916"));
...
void idPlayer::Restore(idRestoreGame* savefile) {
    ...
    // DG: workaround for lingering messages that are shown forever after
    // loading a savegame
    //     (one way to get them is saving again, while the message from first
    //     save is still
    //      shown, and then load)
    if (hud) {
        hud->SetStateString("message", "");
...
void idPlayer::Restart(void) {
    ...
    if (gameLocal.isClient) {
        ...
        // choose a random spot and prepare the point of view in case player is
        // left spectating
        assert(spectating);
        SpawnFromSpawnSpot();
    }
    ...
    if (spectating != spectate) {
        ...
        if (spectate) {
            ...
            if (gameLocal.gameType == GAME_DM) {
                // make sure the scores are reset so you can't exploit by
                // spectating and entering the game back other game types don't
                // matter, as you either can't join back, or it's team scores
...
void idPlayer::SelectInitialSpawnPoint(idVec3& origin, idAngles& angles) {
    ...
    origin[2] += 4.0f + CM_BOX_EPSILON;  // move up to make sure the player is
                                         // at least an epsilon above the floor
...
void idPlayer::SpawnToPoint(const idVec3& spawn_origin,
    ...
    if (gameLocal.isMultiplayer) {
        if (!spectating) {
            // we may be called twice in a row in some situations. avoid a
            // double fx and 'fly to the roof'
            if (lastTeleFX < gameLocal.time - 1000) {
                idEntityFx::StartFx(spawnArgs.GetString("fx_spawn"),
                                    &spawn_origin, NULL, this, true);
    ...
    if (!spectating) {
        physicsObj.SetClipMask(
            MASK_PLAYERSOLID);  // the clip mask is usually maintained in
                                // Move(), but KillBox requires it
...
bool idPlayer::UserInfoChanged(bool canModify) {
    ...
    if (gameLocal.serverInfo.GetBool("si_spectators")) {
        ...
        if (canModify &&
            gameLocal.mpGame.GetGameState() == idMultiplayerGame::SUDDENDEATH &&
            !spec && wantSpectate == true) {
            userInfo->Set("ui_spectate", "Spectate");
            modifiedInfo |= true;
        } else {
            if (spec != wantSpectate && !spec) {
                // returning from spectate, set forceRespawn so we don't get
                // stuck in spectate forever
                forceRespawn = true;
            }
            wantSpectate = spec;
        }
    ...
    if (ready != newready &&
        gameLocal.mpGame.GetGameState() == idMultiplayerGame::WARMUP &&
        !wantSpectate) {
        gameLocal.mpGame.AddChatLine(
            common->GetLanguageDict()->GetString("#str_07180"),
            userInfo->GetString("ui_name"),
            newready ? common->GetLanguageDict()->GetString("#str_04300")
                     : common->GetLanguageDict()->GetString("#str_04301"));
    }
    ready = newready;
    team = (idStr::Icmp(userInfo->GetString("ui_team"), "Blue") == 0);
    // server maintains TDM balance
    if (canModify && gameLocal.gameType == GAME_TDM &&
        !gameLocal.mpGame.IsInGame(entityNumber) && g_balanceTDM.GetBool()) {
...
void idPlayer::UpdateHudAmmo(idUserInterface* _hud) {
    ...
    if (ammoamount < 0 || !weapon.GetEntity()->IsReady()) {
        ...
        _hud->SetStateString("player_ammo",
                             weapon.GetEntity()->ClipSize()
                                 ? va("%i", inclip)
                                 : "--");  // how much in the current clip
        _hud->SetStateString(
            "player_clips",
            weapon.GetEntity()->ClipSize()
                ? va("%i", ammoamount / weapon.GetEntity()->ClipSize())
                : "--");
        _hud->SetStateString("player_allammo",
                             va("%i/%i", inclip, ammoamount - inclip));
    }

    _hud->SetStateBool("player_ammo_empty", (ammoamount == 0));
    _hud->SetStateBool("player_clip_empty",
                       (weapon.GetEntity()->ClipSize() ? inclip == 0 : false));
    _hud->SetStateBool("player_clip_low",
                       (weapon.GetEntity()->ClipSize()
                            ? inclip <= weapon.GetEntity()->LowAmmo()
                            : false));
...
void idPlayer::UpdateHudWeapon(bool flashWeapon) {
    ...
    if (gameLocal.localClientNum >= 0 &&
        gameLocal.entities[gameLocal.localClientNum] &&
        gameLocal.entities[gameLocal.localClientNum]->IsType(idPlayer::Type)) {
        idPlayer* p = static_cast<idPlayer*>(
            gameLocal.entities[gameLocal.localClientNum]);
...
void idPlayer::DrawHUD(idUserInterface* _hud) {
    if (!weapon.GetEntity() || influenceActive != INFLUENCE_NONE ||
        privateCameraView || gameLocal.GetCamera() || !_hud ||
        !g_showHud.GetBool()) {
        return;
    }
    ...
    _hud->SetStateInt("s_debug",
                      cvarSystem->GetCVarInteger("s_showLevelMeter"));
...
void idPlayer::UpdateConditions(void) {
    ...
    AI_RUN = (usercmd.buttons & BUTTON_RUN) &&
             ((!pm_stamina.GetFloat()) ||
...
void idPlayer::FireWeapon(void) {
    ...
    if (!hiddenWeapon && weapon.GetEntity()->IsReady()) {
        if (weapon.GetEntity()->AmmoInClip() ||
...
bool idPlayer::Give(const char* statname, const char* value) {
    ...
    if (!idStr::Icmp(statname, "health")) {
        ...
        return inventory.Give(this, spawnArgs, statname, value, &idealWeapon,
                              true);
    }
    return true;
}
...
bool idPlayer::GiveItem(idItem* item) {
    ...
    if (gave && (numPickup == inventory.pickupItemNames.Num())) {
        inventory.AddPickupName(item->spawnArgs.GetString("inv_name"),
                                item->spawnArgs.GetString("inv_icon"));
    }
    ...
    if (powerup >= 0 && powerup < MAX_POWERUPS) {
        ...
        switch (powerup) {
            case BERSERK: {
                ...
                if (baseSkinName.Length()) {
                    powerUpSkin =
                        declManager->FindSkin(baseSkinName + "_berserk");
                }
                if (!gameLocal.isClient) {
                    idealWeapon = 0;
                }
...
void idPlayer::UpdateObjectiveInfo(void) {
    ...
    for (int i = 0; i < inventory.objectiveNames.Num(); i++) {
        ...
        objectiveSystem->SetStateString(
            va("objectivetitle%i", i + 1),
            inventory.objectiveNames[i].title.c_str());
        objectiveSystem->SetStateString(
            va("objectivetext%i", i + 1),
            inventory.objectiveNames[i].text.c_str());
        objectiveSystem->SetStateString(
            va("objectiveshot%i", i + 1),
            inventory.objectiveNames[i].screenshot.c_str());
    }
...
void idPlayer::GivePDA(const char* pdaName, idDict* item) {
    ...
    const idDeclPDA* pda =
        static_cast<const idDeclPDA*>(declManager->FindType(DECL_PDA, pdaName));
    ...
    // We don't want to display the 'you got a new pda' message during a map
    // load
    if (gameLocal.GetFrameNum() > 10) {
        ...
        if (inventory.pdas.Num() == 1) {
            ...
            // spawnArgs.GetString( "text_firstPDA" ), true );
        }
...
void idPlayer::DropWeapon(bool died) {
    ...
    if (died) {
        ...
        item = weapon.GetEntity()->DropItem(vec3_origin, 0, WEAPON_DROP_TIME,
                                            died);
    } else {
...
void idPlayer::StealWeapon(idPlayer* player) {
    ...
    idWeapon* player_weapon =
        static_cast<idWeapon*>(player->weapon.GetEntity());
    if (!player_weapon || !player_weapon->CanDrop() || weaponGone) {
        return;
    }
    // steal - we need to effectively force the other player to abandon his
    // weapon
...
void idPlayer::Weapon_Combat(void) {
    ...
    if (idealWeapon != currentWeapon) {
        if (weaponCatchup) {
            ...
            weapon.GetEntity()->GetWeaponDef(animPrefix,
                                             inventory.clip[currentWeapon]);
        ...
        weaponGone = false;  // if you drop and re-get weap, you may miss the =
                             // false above
        if (weapon.GetEntity()->IsHolstered()) {
...
void idPlayer::Weapon_GUI(void) {
    ...
    if ((oldButtons ^ usercmd.buttons) & BUTTON_ATTACK) {
        ...
        if (ui) {
            ev = sys->GenerateMouseButtonEvent(
                1, (usercmd.buttons & BUTTON_ATTACK) != 0);
...
void idPlayer::SpectateFreeFly(bool force) {
    ...
    if (force || gameLocal.time > lastSpectateChange) {
        ...
        if (player && player != this && !player->spectating &&
            !player->IsInTeleport()) {
            ...
            gameLocal.clip.TraceBounds(t, start, newOrig, b, MASK_PLAYERSOLID,
                                       player);
    ...
    if (token.Icmp("addhealth") == 0) {
        if (entityGui && health < 100) {
            ...
            if (entityGui->GetRenderEntity() &&
                ...
                entityGui->GetRenderEntity()->gui[0]->SetStateInt("gui_parm1",
                                                                  _health);
            }
            health += amt;
            if (health > 100) {
            ...
            if (mat) {
                ...
                if (pdaVideoWave.Length()) {
                    const idSoundShader* shader =
                        declManager->FindSound(pdaVideoWave);
...
void idPlayer::UpdateLocation(void) {
    if (hud) {
        idLocationEntity* locationEntity =
            gameLocal.LocationForPoint(GetEyePosition());
        if (locationEntity) {
            hud->SetStateString("location", locationEntity->GetLocation());
        } else {
            hud->SetStateString(
                "location", common->GetLanguageDict()->GetString("#str_02911"));
        }
    }
}
...
void idPlayer::UpdateFocus(void) {
    ...
    // only update the focus character when attack button isn't pressed so
    // players can still chainsaw NPC's
    if (gameLocal.isMultiplayer ||
    ...
    if (gameLocal.isMultiplayer && entityNumber == gameLocal.localClientNum) {
        ...
        gameLocal.clip.TracePoint(trace, start, end, MASK_SHOT_BOUNDINGBOX,
                                  this);
        int iclient = -1;
    ...
    // no pretense at sorting here, just assume that there will only be one
    // active gui within range along the trace
        ...
        if (pt.x != -1) {
            ...
            if (oldFocus != ent) {
                ...
                for (j = 0; j < inventory.items.Num(); j++) {
                    ...
                    if (kv) {
                        focusUI->SetStateString(va("inv_id_%i", j),
                                                kv->GetValue());
                    }
    ...
    if (oldChar != focusCharacter && hud) {
        if (focusCharacter) {
            hud->SetStateString(
                "npc", focusCharacter->spawnArgs.GetString("npc_name", "Joe"));
...
void idPlayer::BobCycle(const idVec3& pushVelocity) {
    ...
    if (physicsObj.HasSteppedUp()) {
        ...
        if (deltaTime < STEPUP_TIME) {
            stepUpDelta =
                stepUpDelta * (STEPUP_TIME - deltaTime) / STEPUP_TIME +
...
void idPlayer::UpdateViewAngles(void) {
    ...
    if (!noclip &&
        ...
        // no view changes at all, but we still want to update the deltas or
        // else when we get out of this mode, our view will snap to a kind of
        // random angle
    ...
    for (i = 0; i < 3; i++) {
        ...
        if (influenceActive == INFLUENCE_LEVEL3) {
            ...
            viewAngles[i] = idMath::AngleNormalize180(
    ...
    if (noclip) {
        if (viewAngles.pitch > 89.0f) {
            // don't let the player look down more than 89 degrees while
            // noclipping
            viewAngles.pitch = 89.0f;
        } else if (viewAngles.pitch < -89.0f) {
            // don't let the player look up more than 89 degrees while
            // noclipping
            viewAngles.pitch = -89.0f;
        }
    } else {
...
int idPlayer::GetBaseHeartRate(void) {
    int base =
        idMath::FtoiFast((BASE_HEARTRATE + LOWHEALTH_HEARTRATE_ADJ) -
                         ((float)health / 100.0f) * LOWHEALTH_HEARTRATE_ADJ);
    int rate =
        idMath::FtoiFast(base + (ZEROSTAMINA_HEARTRATE - base) *
...
void idPlayer::SetCurrentHeartRate(void) {
    ...
    if (gameLocal.time - lastHeartBeat > bps) {
        ...
        if (pct != zeroVol) {
            StartSound("snd_heartbeat", SND_CHANNEL_HEART, SSF_PRIVATE_SOUND,
                       false, NULL);
...
void idPlayer::UpdateAir(void) {
    ...
    if (gameLocal.vacuumAreaNum != -1) {
        ...
        if (num > 0) {
            ...
            // if the player box spans multiple areas, get the area from the
            // origin point instead, otherwise a rotating player box may poke
            // into an outside area
            if (num == 1) {
                const int* pvsAreas = GetPVSAreas();
                areaNum = pvsAreas[0];
            } else {
                areaNum = gameRenderWorld->PointInArea(
                    this->GetPhysics()->GetOrigin());
            }
            newAirless = gameRenderWorld->AreasAreConnected(
                gameLocal.vacuumAreaNum, areaNum, PS_BLOCK_AIR);
        }
    }

    if (newAirless) {
        if (!airless) {
            StartSound("snd_decompress", SND_CHANNEL_ANY, SSF_GLOBAL, false,
                       NULL);
        ...
        if (airTics < 0) {
            ...
            int dmgTiming =
                1000 *
                ((damageDef) ? damageDef->GetFloat("delay", "3.0") : 3.0f);
        ...
        if (airless) {
            StartSound("snd_recompress", SND_CHANNEL_ANY, SSF_GLOBAL, false,
                       NULL);
...
const idDeclPDA* idPlayer::GetPDA(void) const {
    if (inventory.pdas.Num()) {
        return static_cast<const idDeclPDA*>(
            declManager->FindType(DECL_PDA, inventory.pdas[0]));
    } else {
        return NULL;
    }
}
...
const idDeclVideo* idPlayer::GetVideo(int index) {
    if (index >= 0 && index < inventory.videos.Num()) {
        return static_cast<const idDeclVideo*>(
            declManager->FindType(DECL_VIDEO, inventory.videos[index], false));
    }
    return NULL;
}
...
void idPlayer::UpdatePDAInfo(bool updatePDASel) {
    ...
    for (j = 0; j < inventory.pdas.Num(); j++) {
        const idDeclPDA* pda = static_cast<const idDeclPDA*>(
            declManager->FindType(DECL_PDA, inventory.pdas[j], false));
        ...
        if (j != currentPDA && j < 128 &&
            inventory.pdasViewed[j >> 5] & (1 << (j & 31))) {
            // This pda has been read already, mark in gray
            objectiveSystem->SetStateString(
                va("listPDA_item_%i", index),
                va(S_COLOR_GRAY "%s", pda->GetPdaName()));
        } else {
            // This pda has not been read yet
            objectiveSystem->SetStateString(va("listPDA_item_%i", index),
                                            pda->GetPdaName());
        }
        ...
        if (j == currentPDA) {
            ...
            if (j == 0) {
                ...
                objectiveSystem->SetStateString(
                    "pda_location", hud->State().GetString("location"));
                objectiveSystem->SetStateString(
                    "pda_name", cvarSystem->GetCVarString("ui_name"));
                AddGuiPDAData(DECL_VIDEO, "listPDAVideo", pda, objectiveSystem);
                sel =
                    objectiveSystem->State().GetInt("listPDAVideo_sel_0", "0");
                const idDeclVideo* vid = NULL;
                if (sel >= 0 && sel < inventory.videos.Num()) {
                    vid = static_cast<const idDeclVideo*>(declManager->FindType(
                        DECL_VIDEO, inventory.videos[sel], false));
                }
                if (vid) {
                    pdaVideo = vid->GetRoq();
                    ...
                    objectiveSystem->SetStateString("PDAVideoTitle",
                                                    vid->GetVideoName());
                    objectiveSystem->SetStateString("PDAVideoVid",
                                                    vid->GetRoq());
                    objectiveSystem->SetStateString("PDAVideoIcon",
                                                    vid->GetPreview());
                    ...
                    objectiveSystem->SetStateString(
                        "PDAVideoVid", "sound/vo/video/welcome.tga");
                    objectiveSystem->SetStateString(
                        "PDAVideoIcon", "sound/vo/video/welcome.tga");
                    objectiveSystem->SetStateString("PDAVideoTitle", "");
                    objectiveSystem->SetStateString("PDAVideoInfo", "");
                }
            } else {
                ...
                int audioCount = AddGuiPDAData(DECL_AUDIO, "listPDAAudio", pda,
                                               objectiveSystem);
                objectiveSystem->SetStateInt("audioLogCount", audioCount);
                sel =
                    objectiveSystem->State().GetInt("listPDAAudio_sel_0", "0");
                ...
                if (aud) {
                    ...
                    objectiveSystem->SetStateString("PDAAudioTitle",
                                                    aud->GetAudioName());
                    objectiveSystem->SetStateString("PDAAudioIcon",
                                                    aud->GetPreview());
                    objectiveSystem->SetStateString("PDAAudioInfo",
                                                    aud->GetInfo());
                } else {
                    objectiveSystem->SetStateString(
                        "PDAAudioIcon", "sound/vo/video/welcome.tga");
            ...
            if (numEmails > 0) {
                ...
                sel =
                    objectiveSystem->State().GetInt("listPDAEmail_sel_0", "-1");
    ...
    if (!objectiveSystemOpen) {
        ...
        for (j = 0; j < c; j++) {
            ...
            if (!item->GetBool("inv_pda")) {
                ...
                if (kv) {
                    objectiveSystem->SetStateString(va("inv_id_%i", j),
                                                    kv->GetValue());
                }
            }
        }
        ...
        inventory.selVideo =
            objectiveSystem->State().GetInt("listPDAVideo_sel_0");
        inventory.selAudio =
            objectiveSystem->State().GetInt("listPDAAudio_sel_0");
        inventory.selEMail =
            objectiveSystem->State().GetInt("listPDAEmail_sel_0");
...
void idPlayer::SetClipModel(void) {
    ...
    if (spectating) {
        ...
        bounds[1].Set(pm_bboxwidth.GetFloat() * 0.5f,
                      pm_bboxwidth.GetFloat() * 0.5f,
                      pm_normalheight.GetFloat());
    }
...
void idPlayer::UseVehicle(void) {
    ...
    if (GetBindMaster() && GetBindMaster()->IsType(idAFEntity_Vehicle::Type)) {
        ...
        gameLocal.clip.TracePoint(trace, start, end, MASK_SHOT_RENDERMODEL,
                                  this);
        if (trace.fraction < 1.0f) {
...
void idPlayer::PerformImpulse(int impulse) {
    ...
    switch (impulse) {
        ...
        case IMPULSE_19: {
            // when we're not in single player, IMPULSE_19 is used for
            // showScores otherwise it opens the pda
            if (!gameLocal.isMultiplayer) {
                if (objectiveSystemOpen) {
                    TogglePDA();
...
void idPlayer::EvaluateControls(void) {
    ...
    if ((usercmd.flags & UCF_IMPULSE_SEQUENCE) !=
        (oldFlags & UCF_IMPULSE_SEQUENCE)) {
        PerformImpulse(usercmd.impulse);
    }

    scoreBoardOpen =
        ((usercmd.buttons & BUTTON_SCORES) != 0 || forceScoreBoard);
...
void idPlayer::AdjustSpeed(void) {
    ...
    if (spectating) {
        ...
        if ((usercmd.forwardmove == 0) && (usercmd.rightmove == 0) &&
            (!physicsObj.OnLadder() || (usercmd.upmove == 0))) {
            rate *= 1.25f;
        }
...
void idPlayer::AdjustBodyAngles(void) {
    ...
    if (!physicsObj.HasGroundContacts()) {
        ...
        idealLegsYaw = idealLegsYaw -
                       idMath::AngleNormalize180(viewAngles.yaw - oldViewYaw);
        if (diff < 0.1f) {
    ...
    animator.CurrentAnim(ANIMCHANNEL_TORSO)
        ->SetSyncedAnimWeight(1, forwardBlend);
    ...
    if (noclip || gameLocal.inCinematic ||
        ...
        if (groundEnt && groundEnt->IsType(idAI::Type)) {
            ...
            if (vel.ToVec2().LengthSqr() < 0.1f) {
                vel.ToVec2() =
                    physicsObj.GetOrigin().ToVec2() - groundEnt->GetPhysics()
                                                          ->GetAbsBounds()
                                                          .GetCenter()
                                                          .ToVec2();
...
void idPlayer::UpdateHud(void) {
    ...
    if (c > 0) {
        if (gameLocal.time > inventory.nextItemPickup) {
            ...
            int i;
            for (i = 0; i < 5 && i < c; i++) {
                hud->SetStateString(va("itemtext%i", inventory.nextItemNum),
                                    inventory.pickupItemNames[0].name);
                hud->SetStateString(va("itemicon%i", inventory.nextItemNum),
                                    inventory.pickupItemNames[0].icon);
                hud->HandleNamedEvent(
                    va("itemPickup%i", inventory.nextItemNum++));
    ...
    if (gameLocal.realClientTime == lastMPAimTime) {
        if (MPAim != -1 && gameLocal.gameType == GAME_TDM &&
            ...
            hud->SetStateString("aim_text",
                                gameLocal.userInfo[MPAim].GetString("ui_name"));
    ...
    if (numProjectilesFired) {
        hud->SetStateString(
            "projectilepct",
            va("Hit %% %.1f",
               ((float)numProjectileHits / numProjectilesFired) * 100));
    } else {
        hud->SetStateString("projectilepct", "Hit % 0.0");
    }

    if (isLagged && gameLocal.isMultiplayer &&
        gameLocal.localClientNum == entityNumber) {
...
void idPlayer::Think(void) {
    ...
    if (weapon.GetEntity()) {
        weapon.GetEntity()->SetPushVelocity(
            physicsObj.GetPushedLinearVelocity());
    }
    ...
    if (!g_stopTime.GetBool()) {
        ...
        // not done on clients for various reasons. don't do it on server and
        // save the sound channel for other things
        if (!gameLocal.isMultiplayer) {
            SetCurrentHeartRate();
        ...
        // clear out our pain flag so we can tell if we recieve any damage
        // between now and the next time we think
        AI_PAIN = false;
    }
...
void idPlayer::Kill(bool delayRespawn, bool nodamage) {
    if (spectating) {
        ...
        if (nodamage) {
            ...
            Damage(this, this, vec3_origin, "damage_suicide", 1.0f,
                   INVALID_JOINT);
            if (delayRespawn) {
                forceRespawn = false;
...
==================
*/
void idPlayer::Killed(idEntity* inflictor,
                      idEntity* attacker,
                      int damage,
                      const idVec3& dir,
                      int location) {
...
=====================
*/
void idPlayer::GetAIAimTargets(const idVec3& lastSightPos,
                               idVec3& headPos,
                               idVec3& chestPos) {
    idVec3 offset;
    idMat3 axis;
...
================
*/
void idPlayer::DamageFeedback(idEntity* victim,
                              idEntity* inflictor,
                              int& damage) {
    assert(!gameLocal.isClient);
...
void idPlayer::CalcDamagePoints(idEntity* inflictor,
    ...
    if (attacker == this) {
        if (gameLocal.isMultiplayer) {
            // only do this in mp so single player plasma and rocket splash is
    ...
    if (!damageDef->GetBool("noArmor")) {
        ...
        armor_protection = (gameLocal.isMultiplayer)
...
============
*/
void idPlayer::Damage(idEntity* inflictor,
                      idEntity* attacker,
                      const idVec3& dir,
    ...
    CalcDamagePoints(inflictor, attacker, &damageDef->dict, damageScale,
    ...
    if (knockback != 0 && !fl.noknockback) {
        if (attacker == this) {
            damageDef->dict.GetFloat("attackerPushScale", "0",
                                     attackerPushScale);
        } else {
...
============
*/
void idPlayer::Teleport(const idVec3& origin,
...
float idPlayer::CalcFov(bool honorZoom) {
    ...
    if (fxFov) {
        return DefaultFov() + 10.0f +
               cos((gameLocal.time + 2000) * 0.01) * 10.0f;
    }
...
void idPlayer::CalculateViewWeaponPos(idVec3& origin, idMat3& axis) {
    ...
    // these cvars are just for hand tweaking before moving a value to the
    // weapon def
...
void idPlayer::OffsetThirdPersonView(float angle,
    ...
    if (clip) {
        ...
        // isn't in a solid block.  Use an 8 by 8 block to prevent the view from
        // near clipping anything
        bounds = idBounds(idVec3(-4, -4, -4), idVec3(4, 4, 4));
        gameLocal.clip.TraceBounds(trace, origin, view, bounds, MASK_SOLID,
                                   this);
        if (trace.fraction != 1.0f) {
            view = trace.endpos;
...
void idPlayer::GetViewPos(idVec3& origin, idMat3& axis) const {
    ...
    if (health <= 0) {
        ...
        // adjust the origin based on the camera nodal distance (eye distance
        // from neck)
...
void idPlayer::CalculateFirstPersonView(void) {
    if ((pm_modelView.GetInteger() == 1) ||
        ...
        //	Displays the view from the point of view of the "camera" joint
        // in the player model
    ...
    if (!noclip && (gameLocal.GetCamera() || privateCameraView)) {
        ...
        if (g_stopTime.GetBool()) {
            ...
            if (!pm_thirdPerson.GetBool()) {
                // set the viewID to the clientNum + 1, so we can suppress the
                // right player bodies and allow the right player view weapons
...
void idPlayer::SetLastHitTime(int time) {
    ...
    if (gameLocal.isMultiplayer && (time - lastSndHitTime) > 10) {
        lastSndHitTime = time;
        StartSound("snd_hit_feedback", SND_CHANNEL_ANY, SSF_PRIVATE_SOUND,
                   false, NULL);
    }
    if (cursor) {
    ...
    if (hud) {
        if (MPAim != -1) {
            ...
            hud->SetStateString(
                "aim_text", gameLocal.userInfo[lastMPAim].GetString("ui_name"));
            if (aimed) {
                hud->SetStateFloat("aim_color", aimed->colorBarIndex);
            }
            hud->HandleNamedEvent("aim_flash");
...
void idPlayer::SetInfluenceView(const char* mtr,
    ...
    if (skinname && *skinname) {
        ...
        if (head.GetEntity()) {
            head.GetEntity()
                ->GetRenderEntity()
                ->shaderParms[SHADERPARM_TIMEOFFSET] = -MS2SEC(gameLocal.time);
        }
        UpdateVisuals();
    }
...
void idPlayer::Event_ExitTeleporter(void) {
    ...
    SetOrigin(exitEnt->GetPhysics()->GetOrigin() +
    ...
    if (teleportKiller != -1) {
        ...
        Damage(gameLocal.entities[teleportKiller],
               gameLocal.entities[teleportKiller], vec3_origin,
...
void idPlayer::ClientPredictionThink(void) {
    ...
    if (gameLocal.framenum >= smoothedFrame &&
        entityNumber != gameLocal.localClientNum) {
        idAngles anglesDiff = viewAngles - smoothedAngles;
        anglesDiff.Normalize180();
        if (idMath::Fabs(anglesDiff.yaw) < 90.0f &&
            idMath::Fabs(anglesDiff.pitch) < 90.0f) {
...
bool idPlayer::GetPhysicsToVisualTransform(idVec3& origin, idMat3& axis) {
    ...
    if (gameLocal.isClient && gameLocal.framenum >= smoothedFrame &&
        (entityNumber != gameLocal.localClientNum || selfSmooth)) {
        // render origin and axis
        idMat3 renderAxis = viewAxis * GetPhysics()->GetAxis();
        idVec3 renderOrigin =
            GetPhysics()->GetOrigin() + modelOffset * renderAxis;
        ...
        if (!smoothedOriginUpdated) {
            ...
            if (originDiff.LengthSqr() < Square(100.0f)) {
                ...
                if (selfSmooth) {
                    ...
                    renderOrigin.ToVec2() -=
                        gameLocal.clientSmoothing * originDiff;
                }
            }
        ...
        origin =
            (smoothedOrigin - GetPhysics()->GetOrigin()) * axis.Transpose();
    ...
    switch (event) {
        ...
        case EVENT_ADD_DAMAGE_EFFECT: {
            if (spectating) {
                ...
                // happens if the event and the spectate change are written on
                // the server during the same frame (fraglimit)
                return true;
            }
            break;
        }
