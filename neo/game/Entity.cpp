...
void idGameEdit::ParseSpawnArgsToRenderEntity(const idDict* args,
    ...
    modelDef = NULL;
    if (temp[0] != '\0') {
        modelDef = static_cast<const idDeclModelDef*>(
            declManager->FindType(DECL_MODELDEF, temp, false));
        if (modelDef) {
    ...
    renderEntity->noDynamicInteractions =
        args->GetBool("noDynamicInteractions");
...
void idEntity::FixupLocalizedStrings() {
    for (int i = 0; i < spawnArgs.GetNumKeyVals(); i++) {
        ...
        if (idStr::Cmpn(kv->GetValue(), STRTABLE_ID, STRTABLE_ID_LENGTH) == 0) {
            spawnArgs.Set(kv->GetKey(),
                          common->GetLanguageDict()->GetString(kv->GetValue()));
        }
    }
}
...
void idEntity::Spawn(void) {
    ...
    if (fl.hidden) {
        // make sure we're hidden, since a spawn function might not set it up
        // right
    ...
    temp = spawnArgs.GetString(
        "name", va("%s_%s_%d", GetClassname(), spawnArgs.GetString("classname"),
                   entityNumber));
    SetName(temp);
    ...
    if (ShouldConstructScriptObjectAtSpawn() &&
        ...
        if (!scriptObject.SetType(scriptObjectName)) {
            gameLocal.Error("Script object '%s' not found on entity '%s'.",
                            scriptObjectName, name.c_str());
        }
...
idEntity::~idEntity(void) {
    if (gameLocal.GameState() != GAMESTATE_SHUTDOWN && !gameLocal.isClient &&
        fl.networkSync && entityNumber >= MAX_CLIENTS) {
        idBitMsg msg;
    ...
    // we have to set back the default physics object before unbinding because
    // the entity specific physics object might be an entity variable and as
    // such could already be destroyed.
    SetPhysics(NULL);
    ...
    if (readsignals) {
        ...
        for (i = 0; i < NUM_SIGNALS; i++) {
            ...
            for (j = 0; j < num; j++) {
                ...
                signals->signal[i][j].function =
                    gameLocal.program.FindFunction(funcname);
                if (!signals->signal[i][j].function) {
                    savefile->Error("Function '%s' not found",
                                    funcname.c_str());
                }
            }
        }
...
const char* idEntity::GetEntityDefName(void) const {
    ...
    return declManager->DeclByIndex(DECL_ENTITYDEF, entityDefNumber, false)
        ->GetName();
}
...
void idEntity::SetName(const char* newname) {
    ...
    if (name.Length()) {
        if ((name == "NULL") || (name == "null_entity")) {
            gameLocal.Error(
                "Cannot name entity '%s'.  '%s' is reserved for script.",
...
bool idEntity::DoDormantTests(void) {
    ...
    dormantStart = 0;
    fl.hasAwakened =
        true;  // only go dormant when area closed off now, not just out of PVS
...
void idEntity::BecomeActive(int flags) {
    if ((flags & TH_PHYSICS)) {
        ...
        if (teamMaster && teamMaster != this) {
            ...
            // if this is a pusher
            if (physics->IsType(idPhysics_Parametric::Type) ||
                physics->IsType(idPhysics_Actor::Type)) {
    ...
    if (thinkFlags) {
        if (!IsActive()) {
            ...
            // we became inactive this frame, so we have to decrease the count
            // of entities to deactivate
...
void idEntity::BecomeInactive(int flags) {
    if ((flags & TH_PHYSICS)) {
        // may only disable physics on a team master if no team members are
        // running physics or bound to a joints
        if (teamMaster == this) {
            for (idEntity* ent = teamMaster->teamChain; ent;
                 ent = ent->teamChain) {
                if ((ent->thinkFlags & TH_PHYSICS) ||
                    ((ent->bindMaster == this) &&
                     (ent->bindJoint != INVALID_JOINT))) {
                    flags &= ~TH_PHYSICS;
                    break;
                }
            }
...
void idEntity::UpdatePVSAreas(void) {
    ...
    modelAbsBounds.FromTransformedBounds(
        renderEntity.bounds, renderEntity.origin, renderEntity.axis);
    localNumPVSAreas = gameLocal.pvs.GetPVSAreas(
        modelAbsBounds, localPVSAreas,
        sizeof(localPVSAreas) / sizeof(localPVSAreas[0]));

    // FIXME: some particle systems may have huge bounds and end up in many PVS
    // areas the first MAX_PVS_AREAS may not be visible to a network client and
    // as a result the particle system may not show up when it should
    if (localNumPVSAreas > MAX_PVS_AREAS) {
        localNumPVSAreas = gameLocal.pvs.GetPVSAreas(
            idBounds(modelAbsBounds.GetCenter()).Expand(64.0f), localPVSAreas,
            sizeof(localPVSAreas) / sizeof(localPVSAreas[0]));
    }

    for (numPVSAreas = 0;
         numPVSAreas < MAX_PVS_AREAS && numPVSAreas < localNumPVSAreas;
         numPVSAreas++) {
...
void idEntity::UpdatePVSAreas(const idVec3& pos) {
    ...
    numPVSAreas =
        gameLocal.pvs.GetPVSAreas(idBounds(pos), PVSAreas, MAX_PVS_AREAS);
    i = numPVSAreas;
...
bool idEntity::PhysicsTeamInPVS(pvsHandle_t pvsHandle) {
    ...
    if (teamMaster) {
        for (part = teamMaster; part; part = part->teamChain) {
            if (gameLocal.pvs.InCurrentPVS(pvsHandle, part->GetPVSAreas(),
                                           part->GetNumPVSAreas())) {
                return true;
            }
        }
    } else {
        return gameLocal.pvs.InCurrentPVS(pvsHandle, GetPVSAreas(),
                                          GetNumPVSAreas());
    }
    return false;
}
...
bool idEntity::StartSound(const char* soundName,
    ...
    return StartSoundShader(shader, channel, soundShaderFlags, broadcast,
                            length);
}
...
bool idEntity::StartSoundShader(const idSoundShader* shader,
    ...
    if (gameLocal.isServer && broadcast) {
        ...
        msg.BeginWriting();
        msg.WriteInt(
            gameLocal.ServerRemapDecl(-1, DECL_SOUND, shader->Index()));
    ...
    len = refSound.referenceSound->StartSound(shader, channel, diversity,
                                              soundShaderFlags);
    if (length) {
        *length = len;
    }
...
void idEntity::UpdateSound(void) {
    if (refSound.referenceSound) {
        ...
        refSound.referenceSound->UpdateEmitter(
            refSound.origin, refSound.listenerId, &refSound.parms);
    }
}
...
void idEntity::BindToJoint(idEntity* master,
    ...
    if (!masterAnimator) {
        gameLocal.Warning(
            "idEntity::BindToJoint: entity '%s' cannot support skeletal "
            "models.",
            master->GetName());
        return;
    }
...
void idEntity::Unbind(void) {
    ...
    prev = teamMaster;
    for (ent = teamMaster->teamChain; ent && (ent != this);
         ent = ent->teamChain) {
        prev = ent;
    }
    ...
    assert(
        ent ==
        this);  // If ent is not pointing to this, then something is very wrong.
    ...
    if (teamMaster != this) {
        ...
        // If we were the teamMaster, then the nodes that were not bound to me
...
void idEntity::GetWorldVelocities(idVec3& linearVelocity,
    ...
    if (bindMaster) {
        ...
        bindMaster->GetWorldVelocities(masterLinearVelocity,
                                       masterAngularVelocity);
        ...
        linearVelocity = linearVelocity * masterAxis + masterLinearVelocity +
                         masterAngularVelocity.Cross(GetPhysics()->GetOrigin() -
                                                     masterOrigin);
    }
}
...
void idEntity::JoinTeam(idEntity* teammember) {
    ...
    if (!master) {
        ...
        if (bindMaster) {
            // if we have a bindMaster, join after any entities bound to the
        ...
        if (!clipModel) {
            ...
            if (spawnArgs.GetVector("mins", NULL, bounds[0]) &&
                ...
                if (bounds[0][0] > bounds[1][0] ||
                    ...
                    gameLocal.Error("Invalid bounds '%s'-'%s' on entity '%s'",
                                    bounds[0].ToString(), bounds[1].ToString(),
                                    name.c_str());
                }
            } else if (spawnArgs.GetVector("size", NULL, size)) {
                if ((size.x < 0.0f) || (size.y < 0.0f) || (size.z < 0.0f)) {
                    gameLocal.Error("Invalid size '%s' on entity '%s'",
                                    size.ToString(), name.c_str());
                }
...
bool idEntity::RunPhysics(void) {
    ...
    // save the physics state of the whole team and disable the team for
    // collision detection
    ...
    for (part = this; part != NULL; part = part->teamChain) {
        if (part->physics) {
            ...
            // if moved or forced to update the visual position and orientation
            // from the physics
...
void idEntity::UpdateFromPhysics(bool moveBack) {
    if (IsType(idActor::Type)) {
        ...
        if (GetBindMaster()) {
            ...
            if (moveBack) {
                delta.yaw -=
                    static_cast<idPhysics_Actor*>(physics)->GetMasterDeltaYaw();
            } else {
                delta.yaw +=
                    static_cast<idPhysics_Actor*>(physics)->GetMasterDeltaYaw();
            }
...
bool idEntity::GetFloorPos(float max_dist, idVec3& floorpos) const {
    ...
    if (!GetPhysics()->HasGroundContacts()) {
        GetPhysics()->ClipTranslation(
            result, GetPhysics()->GetGravityNormal() * max_dist, NULL);
...
bool idEntity::CanDamage(const idVec3& origin, idVec3& damagePoint) const {
    ...
    midpoint =
        (GetPhysics()->GetAbsBounds()[0] + GetPhysics()->GetAbsBounds()[1]) *
        0.5;
...
void idEntity::Damage(idEntity* inflictor,
    ...
    if (damage) {
        ...
        gameRenderWorld->DrawText(
            va("%d", damage), static_cast<idActor*>(this)->GetEyePosition(),
            0.25, colorWhite, gameLocal.GetLocalPlayer()->viewAngles.ToMat3());
        if (health <= 0) {
            if (health < -999) {
                health = -999;
            }
...
void idEntity::AddDamageEffect(const trace_t& collision,
    ...
    if (*sound != '\0') {
        StartSoundShader(declManager->FindSound(sound), SND_CHANNEL_BODY, 0,
                         false, NULL);
    }
    ...
    if (entityGui && cmds && *cmds) {
        ...
        while (1) {
                ...
                if (src.ReadToken(&token2)) {
                    while (src.CheckTokenString("::")) {
                        ...
                        if (!src.ReadToken(&token3)) {
                            gameLocal.Error(
                                "Expecting function name following '::' in gui "
                                "for entity '%s'",
                    ...
                    const function_t* func =
                        gameLocal.program.FindFunction(token2);
                    if (!func) {
                        gameLocal.Error(
                            "Can't find function '%s' for gui in entity '%s'",
                            token2.c_str(), entityGui->name.c_str());
                    } else {
            ...
            if (token.Icmp("play") == 0) {
                if (src.ReadToken(&token2)) {
                    const idSoundShader* shader =
                        declManager->FindSound(token2);
                    entityGui->StartSoundShader(shader, SND_CHANNEL_ANY, 0,
                                                false, NULL);
                }
                continue;
            }
            ...
            if (!token.Icmp("turkeyscore")) {
                if (src.ReadToken(&token2) && entityGui->renderEntity.gui[0]) {
                    int score =
                        entityGui->renderEntity.gui[0]->State().GetInt("score");
                    score += atoi(token2);
                    entityGui->renderEntity.gui[0]->SetStateInt("score", score);
                    if (gameLocal.GetLocalPlayer() && score >= 25000 &&
                        !gameLocal.GetLocalPlayer()->inventory.turkeyScore) {
            ...
            if (!HandleSingleGuiCommand(entityGui, &src)) {
                // not handled there see if entity or any of its targets can
                // handle it this will only work for one target atm
                ...
                for (i = 0; i < c; i++) {
                    ...
                    if (targetEnt &&
                        targetEnt->HandleSingleGuiCommand(entityGui, &src)) {
                        break;
                    }
                }
                ...
                if (i == c) {
                    // not handled
                    common->DPrintf(
                        "idEntity::HandleGuiCommands: '%s' not handled\n",
                        token.c_str());
                    src.ReadToken(&token);
                }
            }
...
bool idEntity::TouchTriggers(void) const {
    ...
    numClipModels = gameLocal.clip.ClipModelsTouchingBounds(
        GetPhysics()->GetAbsBounds(), CONTENTS_TRIGGER, clipModels,
        MAX_GENTITIES);
    numEntities = 0;
...
void idEntity::Event_SpawnBind(void) {
    ...
    if (spawnArgs.GetString("bind", "", &bind)) {
        if (idStr::Icmp(bind, "worldspawn") == 0) {
            // FIXME: Completely unneccessary since the worldspawn is called
            // "world"
                ...
                if ((parent->spawnArgs.GetString("bindanim", "", &bindanim) ||
                    ...
                    *bindanim) {
                    ...
                    gameEdit->ANIM_CreateAnimFrame(
                        parentAnimator->ModelHandle(), anim->MD5Anim(0),
...
void idEntity::Event_StartSoundShader(const char* soundName, int channel) {
    ...
    StartSoundShader(declManager->FindSound(soundName), (s_channelType)channel,
                     0, false, &length);
...
void idEntity::Event_RestorePosition(void) {
    ...
    for (part = teamChain; part != NULL; part = part->teamChain) {
        ...
        if (part->GetPhysics()->IsType(idPhysics_Parametric::Type)) {
            if (static_cast<idPhysics_Parametric*>(part->GetPhysics())
                    ->IsPusher()) {
                gameLocal.Warning(
                    "teleported '%s' which has the pushing mover '%s' bound to "
                    "it\n",
                    GetName(), part->GetName());
                gameLocal.Warning(
                    "  from (%.2f %.2f %.2f) to (%.2f %.2f %.2f)\n", oldOrg.x,
                    oldOrg.y, oldOrg.z, org.x, org.y, org.z);
            }
        } else if (part->GetPhysics()->IsType(idPhysics_AF::Type)) {
            gameLocal.Warning(
                "teleported '%s' which has the articulated figure '%s' bound "
                "to it\n",
                GetName(), part->GetName());
        }
    }
...
void idEntity::Event_UpdateCameraTarget(void) {
    ...
    if (cameraTarget) {
        ...
        while (kv) {
            ...
            if (ent &&
                idStr::Icmp(ent->GetEntityDefName(), "target_null") == 0) {
                dir = ent->GetPhysics()->GetOrigin() -
                      cameraTarget->GetPhysics()->GetOrigin();
                dir.Normalize();
...
void idEntity::Event_DistanceTo(idEntity* ent) {
    if (!ent) {
        ...
        float dist =
            (GetPhysics()->GetOrigin() - ent->GetPhysics()->GetOrigin())
                .LengthFast();
...
void idEntity::Event_CallFunction(const char* funcname) {
    ...
    if (!func) {
        gameLocal.Error("Unknown function '%s' in '%s'", funcname,
                        scriptObject.GetTypeName());
    }

    if (func->type->NumParameters() != 1) {
        gameLocal.Error(
            "Function '%s' has the wrong number of parameters for "
            "'callFunction'",
            funcname);
    }
    if (!scriptObject.GetTypeDef()->Inherits(func->type->GetParmType(0))) {
        gameLocal.Error("Function '%s' is the wrong type for 'callFunction'",
                        funcname);
    }
...
void idEntity::ServerSendEvent(int eventId,
    ...
    if (msg) {
        outMsg.WriteBits(msg->GetSize(),
                         idMath::BitsForInteger(MAX_EVENT_PARAM_SIZE));
    ...
    if (excludeClient != -1) {
        networkSystem->ServerSendReliableMessageExcluding(excludeClient,
                                                          outMsg);
    } else {
    ...
    switch (event) {
        case EVENT_STARTSOUNDSHADER: {
            ...
            if (time < gameLocal.realClientTime - 1000) {
                // too old, skip it ( reliable messages don't need to be parsed
                // in full )
                common->DPrintf(
...
=====================
*/
bool idAnimatedEntity::GetJointWorldTransform(jointHandle_t jointHandle,
                                              int currentTime,
                                              idVec3& offset,
                                              idMat3& axis) {
...
==============
*/
bool idAnimatedEntity::GetJointTransformForAnim(jointHandle_t jointHandle,
                                                int animNum,
                                                int frameTime,
                                                idVec3& offset,
                                                idMat3& axis) const {
    ...
    gameEdit->ANIM_CreateAnimFrame(animator.ModelHandle(), anim->MD5Anim(0),
                                   renderEntity.numJoints, frame, frameTime,
...
==============
*/
void idAnimatedEntity::AddDamageEffect(const trace_t& collision,
                                       const idVec3& velocity,
                                       const char* damageDefName) {
    jointHandle_t jointNum;
    ...
    origin = renderEntity.origin +
             renderEntity.joints[jointNum].ToVec3() * renderEntity.axis;
    ...
    AddLocalDamageEffect(jointNum, localOrigin, localNormal, localDir, def,
                         collision.c.material);
    ...
    if (gameLocal.isServer) {
        ...
        msg.WriteInt(
            gameLocal.ServerRemapDecl(-1, DECL_ENTITYDEF, def->Index()));
        msg.WriteInt(gameLocal.ServerRemapDecl(-1, DECL_MATERIAL,
...
void idAnimatedEntity::AddLocalDamageEffect(
    ...
    const idVec3& localNormal,
    const idVec3& localDir,
    const idDeclEntityDef* def,
    const idMaterial* collisionMaterial) {
...
void idAnimatedEntity::UpdateDamageEffects(void) {
    ...
    for (de = this->damageEffects; de; de = de->next) {
        ...
        if (!gameLocal.smokeParticles->EmitSmoke(
                de->type, de->time, gameLocal.random.CRandomFloat(), start,
                axis)) {
            de->time = 0;
        }
    }
...
================
*/
bool idAnimatedEntity::ClientReceiveEvent(int event,
                                          int time,
                                          const idBitMsg& msg) {
    int damageDefIndex;
    int materialIndex;
    ...
    switch (event) {
        case EVENT_ADD_DAMAGE_EFFECT: {
            ...
            damageDefIndex =
                gameLocal.ClientRemapDecl(DECL_ENTITYDEF, msg.ReadInt());
            materialIndex =
                gameLocal.ClientRemapDecl(DECL_MATERIAL, msg.ReadInt());
            const idDeclEntityDef* damageDef =
                static_cast<const idDeclEntityDef*>(
                    declManager->DeclByIndex(DECL_ENTITYDEF, damageDefIndex));
            const idMaterial* collisionMaterial =
                static_cast<const idMaterial*>(
                    declManager->DeclByIndex(DECL_MATERIAL, materialIndex));
            AddLocalDamageEffect(jointNum, localOrigin, localNormal, localDir,
                                 damageDef, collisionMaterial);
            return true;
        }
        default:
            break;
...
================
*/
void idAnimatedEntity::Event_SetJointPos(jointHandle_t jointnum,
...
================
*/
void idAnimatedEntity::Event_SetJointAngle(jointHandle_t jointnum,
                                           jointModTransform_t transform_type,
                                           const idAngles& angles) {
    idMat3 mat;
...
void idAnimatedEntity::Event_GetJointPos(jointHandle_t jointnum) {
    ...
    if (!GetJointWorldTransform(jointnum, gameLocal.time, offset, axis)) {
        gameLocal.Warning("Joint # %d out of range on entity '%s'", jointnum,
                          name.c_str());
    }
