...
class idInventory {
   public:
    ...
    // save games
    void Save(
        idSaveGame* savefile) const;  // archives object for save game file
    void Restore(
        idRestoreGame* savefile);  // unarchives object from save game file
    ...
    bool Give(idPlayer* owner,
              const idDict& spawnArgs,
              const char* statname,
              const char* value,
              int* idealWeapon,
              bool updateHud);
    void Drop(const idDict& spawnArgs,
    ...
    int HasAmmo(const char* weapon_classname);  // looks up the ammo information
                                                // for the weapon class first
...
class idPlayer : public idActor {
   public:
    ...
    bool useInitialSpawns;  // toggled by a map restart to be active for the
                            // first game spawn
    int latchedTeam;        // need to track when team gets changed
    int tourneyRank;  // for tourney cycling - the higher, the more likely to
                      // play next - server
    int tourneyLine;  // client side - our spot in the wait line. 0 means no
                      // info.
    int spawnedTime;  // when client first enters the game

    idEntityPtr<idEntity>
        teleportEntity;  // while being teleported, this is set to the entity
                         // we'll use for exit
    ...
    bool lastManPresent;    // true when player was in when game started
                            // (spectators can't join a running LMS)
   ...
   public:
    ...
    virtual void GetAIAimTargets(const idVec3& lastSightPos,
                                 idVec3& headPos,
                                 idVec3& chestPos);
    virtual void DamageFeedback(idEntity* victim,
                                idEntity* inflictor,
                                int& damage);
    void CalcDamagePoints(idEntity* inflictor,
                          idEntity* attacker,
    ...
    void OffsetThirdPersonView(float angle,
                               float range,
                               float height,
                               bool clip);
    ...
    void GiveObjective(const char* title,
                       const char* text,
    ...
    void SetInfluenceView(const char* mtr,
                          const char* skinname,
                          float radius,
                          idEntity* ent);
    ...
    // server side work for in/out of spectate. takes care of spawning it into
    // the world as well
    ...
    // KOLEGA PLAYER
    float get_ruler_distance() const { return ruler_distance; }
    float get_ruler_beeline_distance() const { return ruler_beeline_distance; }
   ...
   private:
    ...
    float influenceFov;
    int influenceActive;  // level of influence.. 1 == no gun or hud .. 2 == 1 +
                          // no movement
    ...
    static const int NUM_LOGGED_ACCELS =
        16;  // for weapon turning angle offsets
    ...
    idAI* focusCharacter;
    int talkCursor;  // show the state of the focusCharacter (0 == can't
                     // talk/dead, 1 == ready to talk, 2 == busy talking)
    int focusTime;
    ...
    // KOLEGA PLAYER VARIABLES
    float ruler_distance;
    float ruler_beeline_distance;
    ...
    void SpectateFreeFly(bool force);  // ignore the timeout to force when
                                       // followed spec is no longer valid
    ...
    int AddGuiPDAData(const declType_t dataType,
                      const char* listName,
                      const idDeclPDA* src,
                      idUserInterface* gui);
