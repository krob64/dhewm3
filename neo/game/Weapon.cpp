...
extern bool
IsDoom3DemoVersion();  // DG: hack to support the Demo version of Doom3
...
void idWeapon::SetOwner(idPlayer* _owner) {
    ...
    if (worldModel.GetEntity()) {
        worldModel.GetEntity()->SetName(
            va("%s_weapon_worldmodel", owner->name.c_str()));
    }
}
...
void idWeapon::CacheWeapon(const char* weaponName) {
    ...
    if (brassDefName[0]) {
        const idDeclEntityDef* brassDef =
            gameLocal.FindEntityDef(brassDefName, false);
        if (brassDef) {
            brassDef->dict.GetString("clipmodel", "", &clipModelName);
            if (!clipModelName[0]) {
                clipModelName =
                    brassDef->dict.GetString("model");  // use the visual model
            }
...
void idWeapon::Restore(idRestoreGame* savefile) {
    ...
    if (!IsDoom3DemoVersion())  // the demo assets don't support
                                // WEAPON_NETFIRING
    ...
    meleeDef =
        gameLocal.FindEntityDef(weaponDef->dict.GetString("def_melee"), false);

    const idDeclEntityDef* projectileDef = gameLocal.FindEntityDef(
        weaponDef->dict.GetString("def_projectile"), false);
    if (projectileDef) {
    ...
    const idDeclEntityDef* brassDef = gameLocal.FindEntityDef(
        weaponDef->dict.GetString("def_ejectBrass"), false);
    if (brassDef) {
    ...
    // DG: we need to get a fresh handle, otherwise this will be tied to a
    // completely unrelated light!
    if (guiLightHandle != -1) {
...
void idWeapon::InitWorldModel(const idDeclEntityDef* def) {
    ...
    if (model[0] && attach[0]) {
        ...
        // supress model in player views, but allow it in mirrors and remote
        // views
        renderEntity_t* worldModelRenderEntity = ent->GetRenderEntity();
        if (worldModelRenderEntity) {
            worldModelRenderEntity->suppressSurfaceInViewID =
                owner->entityNumber + 1;
            worldModelRenderEntity->suppressShadowInViewID =
                owner->entityNumber + 1;
...
void idWeapon::GetWeaponDef(const char* objectname, int ammoinclip) {
    ...
    muzzle_kick_maxtime =
        SEC2MS(weaponDef->dict.GetFloat("muzzle_kick_maxtime"));
    ...
    if (*smokeName != '\0') {
        weaponSmoke = static_cast<const idDeclParticle*>(
    ...
    if (*smokeName != '\0') {
        strikeSmoke = static_cast<const idDeclParticle*>(
            declManager->FindType(DECL_PARTICLE, smokeName));
    } else {
        strikeSmoke = NULL;
    }
    ...
    // setup gui light
    memset(&guiLight, 0, sizeof(guiLight));
    const char* guiLightShader =
        weaponDef->dict.GetString("mtr_guiLightShader");
    if (*guiLightShader != '\0') {
        guiLight.shader = declManager->FindMaterial(guiLightShader, false);
        guiLight.lightRadius[0] = guiLight.lightRadius[1] =
            guiLight.lightRadius[2] = 3;
    ...
    if (projectileName[0] != '\0') {
        ...
        if (!projectileDef) {
            ...
            if (!cls || !cls->IsType(idProjectile::Type)) {
                gameLocal.Warning(
                    "Invalid spawnclass '%s' on projectile '%s' (used by "
                    "weapon '%s')",
                    spawnclass, projectileName, objectname);
            } else {
                projectileDict = projectileDef->dict;
            }
        }
    ...
    flashRadius = (float)weaponDef->dict.GetInt(
        "flashRadius");  // if 0, no light will spawn
    ...
    weaponAngleOffsetAverages =
        weaponDef->dict.GetInt("weaponAngleOffsetAverages", "10");
    weaponAngleOffsetScale =
        weaponDef->dict.GetFloat("weaponAngleOffsetScale", "0.25");
    weaponAngleOffsetMax =
        weaponDef->dict.GetFloat("weaponAngleOffsetMax", "10");
    ...
    if (!scriptObject.SetType(objectType)) {
        gameLocal.Error("Script object '%s' not found on weapon '%s'.",
...
void idWeapon::UpdateGUI(void) {
    ...
    if (owner->weaponGone) {
        // dropping weapons was implemented wierd, so we have to not update the
        // gui when it happens or we'll get a negative ammo count
        return;
    }
    ...
    if (gameLocal.localClientNum != owner->entityNumber) {
        ...
        if (gameLocal.localClientNum >= 0 &&
            ...
            gameLocal.entities[gameLocal.localClientNum]->IsType(
                idPlayer::Type)) {
    ...
    if (ammoamount < 0) {
        ...
        // show remaining ammo
        renderEntity.gui[0]->SetStateString("player_totalammo",
                                            va("%i", ammoamount - inclip));
        renderEntity.gui[0]->SetStateString(
            "player_ammo", ClipSize() ? va("%i", inclip) : "--");
        renderEntity.gui[0]->SetStateString(
            "player_clips",
            ClipSize() ? va("%i", ammoamount / ClipSize()) : "--");
        renderEntity.gui[0]->SetStateString(
            "player_allammo", va("%i/%i", inclip, ammoamount - inclip));
    }
...
void idWeapon::UpdateFlashPosition(void) {
    ...
    GetGlobalJointTransform(true, flashJointView, muzzleFlash.origin,
                            muzzleFlash.axis);

    // if the desired point is inside or very close to a wall, back it up until
    // it is clear
    ...
    GetGlobalJointTransform(false, flashJointWorld, worldMuzzleFlash.origin,
...
bool idWeapon::UpdateSkin(void) {
    ...
    if (!func) {
        common->Warning("Can't find function 'UpdateSkin' in object '%s'",
                        scriptObject.GetTypeName());
        return false;
    }

    // use the frameCommandThread since it's safe to use outside of
    // framecommands
...
bool idWeapon::GetGlobalJointTransform(bool viewModel,
    ...
    if (viewModel) {
        // view model
        if (animator.GetJointTransform(jointHandle, gameLocal.time, offset,
                                       axis)) {
        ...
        // world model
        if (worldModel.GetEntity() &&
            worldModel.GetEntity()->GetAnimator()->GetJointTransform(
                jointHandle, gameLocal.time, offset, axis)) {
            offset = worldModel.GetEntity()->GetPhysics()->GetOrigin() +
                     offset * worldModel.GetEntity()->GetPhysics()->GetAxis();
...
void idWeapon::LowerWeapon(void) {
    if (!hide) {
        ...
        if (gameLocal.time - hideStartTime < hideTime) {
            hideStartTime =
                gameLocal.time - (hideTime - (gameLocal.time - hideStartTime));
        } else {
...
bool idWeapon::IsReady(void) const {
    return !hide && !IsHidden() &&
           ((status == WP_RELOAD) || (status == WP_READY) ||
            (status == WP_OUTOFAMMO));
}
...
bool idWeapon::ShowCrosshair(void) const {
    return !(state == idStr(WP_RISING) || state == idStr(WP_LOWERING) ||
             state == idStr(WP_HOLSTERED));
}
...
idEntity* idWeapon::DropItem(const idVec3& velocity,
    ...
    return idMoveableItem::DropItem(
        classname, worldModel.GetEntity()->GetPhysics()->GetOrigin(),
        worldModel.GetEntity()->GetPhysics()->GetAxis(), velocity,
        activateDelay, removeDelay);
}
...
void idWeapon::SetState(const char* statename, int blendFrames) {
    ...
    if (!func) {
        assert(0);
        gameLocal.Error("Can't find function '%s' in object '%s'", statename,
                        scriptObject.GetTypeName());
    }
...
void idWeapon::UpdateNozzleFx(void) {
    ...
    if (nozzleGlowHandle == -1) {
        ...
        GetGlobalJointTransform(true, ventLightJointView, nozzleGlow.origin,
                                nozzleGlow.axis);
        nozzleGlowHandle = gameRenderWorld->AddLightDef(&nozzleGlow);
    }
...
bool idWeapon::BloodSplat(float size) {
    ...
    if (!GetGlobalJointTransform(true, ejectJointView, localOrigin,
                                 localAxis)) {
        return false;
    }
    ...
    normal = idVec3(gameLocal.random.CRandomFloat(),
                    -gameLocal.random.RandomFloat(), -1);
    normal.Normalize();
...
idThread* idWeapon::ConstructScriptObject(void) {
    ...
    if (!constructor) {
        gameLocal.Error("Missing constructor on '%s' for weapon",
                        scriptObject.GetTypeName());
    }
...
void idWeapon::AlertMonsters(void) {
    ...
    gameLocal.clip.TracePoint(
        tr, muzzleFlash.origin, end,
        CONTENTS_OPAQUE | MASK_SHOT_RENDERMODEL | CONTENTS_FLASHLIGHT_TRIGGER,
        owner);
    if (g_debugWeapon.GetBool()) {
        gameRenderWorld->DebugLine(colorYellow, muzzleFlash.origin, end, 0);
        gameRenderWorld->DebugArrow(colorGreen, muzzleFlash.origin, tr.endpos,
                                    2, 0);
    }
    ...
    end += muzzleFlash.axis * muzzleFlash.right *
           idMath::Sin16(MS2SEC(gameLocal.time) * 31.34f);
    end += muzzleFlash.axis * muzzleFlash.up *
           idMath::Sin16(MS2SEC(gameLocal.time) * 12.17f);
    gameLocal.clip.TracePoint(
        tr, muzzleFlash.origin, end,
        CONTENTS_OPAQUE | MASK_SHOT_RENDERMODEL | CONTENTS_FLASHLIGHT_TRIGGER,
        owner);
...
void idWeapon::PresentWeapon(bool showViewModel) {
    ...
    // crunch the depth range so it never pokes into walls this breaks the
    // machine gun gui
    ...
    if (worldModel.GetEntity() && worldModel.GetEntity()->GetRenderEntity()) {
        ...
        if (gameLocal.isMultiplayer || g_showPlayerShadow.GetBool() ||
            ...
            worldModel.GetEntity()->GetRenderEntity()->suppressShadowInViewID =
                0;
        } else {
            worldModel.GetEntity()->GetRenderEntity()->suppressShadowInViewID =
                owner->entityNumber + 1;
    ...
    // muzzle smoke
    if (showViewModel && !disabled && weaponSmoke &&
        (weaponSmokeStartTime != 0)) {
        // use the barrel joint if available
        if (barrelJointView) {
            GetGlobalJointTransform(true, barrelJointView, muzzleOrigin,
                                    muzzleAxis);
        } else {
        ...
        if (!gameLocal.smokeParticles->EmitSmoke(
                weaponSmoke, weaponSmokeStartTime,
                gameLocal.random.RandomFloat(), muzzleOrigin, muzzleAxis)) {
            weaponSmokeStartTime = (continuousSmoke) ? gameLocal.time : 0;
        }
    }
    ...
    if (showViewModel && strikeSmoke && strikeSmokeStartTime != 0) {
        // spit out a particle
        if (!gameLocal.smokeParticles->EmitSmoke(
                strikeSmoke, strikeSmokeStartTime,
                gameLocal.random.RandomFloat(), strikePos, strikeAxis)) {
            strikeSmokeStartTime = 0;
        }
    }
    ...
    if (guiLight.lightRadius[0] && guiLightJointView != INVALID_JOINT) {
        GetGlobalJointTransform(true, guiLightJointView, guiLight.origin,
                                guiLight.axis);
...
void idWeapon::ReadFromSnapshot(const idBitMsgDelta& msg) {
    ...
    // WEAPON_NETFIRING is only turned on for other clients we're predicting.
    // not for local client
    if (owner && gameLocal.localClientNum != owner->entityNumber &&
        WEAPON_NETFIRING.IsLinked()) {
...
bool idWeapon::ClientReceiveEvent(int event, int time, const idBitMsg& msg) {
    switch (event) {
        ...
        case EVENT_CHANGESKIN: {
            ...
            renderEntity.customSkin =
                (index != -1) ? static_cast<const idDeclSkin*>(
                                    declManager->DeclByIndex(DECL_SKIN, index))
                              : NULL;
            UpdateVisuals();
            if (worldModel.GetEntity()) {
...
void idWeapon::Event_UseAmmo(int amount) {
    ...
    owner->inventory.UseAmmo(ammoType,
                             (powerAmmo) ? amount : (amount * ammoRequired));
...
void idWeapon::Event_AutoReload(void) {
    ...
    idThread::ReturnFloat(
        gameLocal.userInfo[owner->entityNumber].GetBool("ui_autoReload"));
}
...
void idWeapon::Event_PlayAnim(int channel, const char* animname) {
    ...
    if (!anim) {
        gameLocal.Warning("missing '%s' animation on '%s' (%s)", animname,
        ...
        animator.PlayAnim(channel, anim, gameLocal.time,
                          FRAME2MS(animBlendFrames));
        animDoneTime = animator.CurrentAnim(channel)->GetEndTime();
        if (worldModel.GetEntity()) {
            anim = worldModel.GetEntity()->GetAnimator()->GetAnim(animname);
            if (anim) {
                worldModel.GetEntity()->GetAnimator()->PlayAnim(
                    channel, anim, gameLocal.time, FRAME2MS(animBlendFrames));
            }
        }
    }
    animBlendFrames = 0;
...
void idWeapon::Event_PlayCycle(int channel, const char* animname) {
    ...
    if (!anim) {
        ...
        animator.CycleAnim(channel, anim, gameLocal.time,
                           FRAME2MS(animBlendFrames));
        animDoneTime = animator.CurrentAnim(channel)->GetEndTime();
        if (worldModel.GetEntity()) {
            anim = worldModel.GetEntity()->GetAnimator()->GetAnim(animname);
            worldModel.GetEntity()->GetAnimator()->CycleAnim(
                channel, anim, gameLocal.time, FRAME2MS(animBlendFrames));
        }
    }
    animBlendFrames = 0;
...
void idWeapon::Event_SetSkin(const char* skinname) {
    ...
    if (gameLocal.isServer) {
        ...
        msg.WriteInt((skinDecl != NULL) ? gameLocal.ServerRemapDecl(
                                              -1, DECL_SKIN, skinDecl->Index())
                                        : -1);
...
void idWeapon::Event_LaunchProjectiles(int num_projectiles,
    ...
    if (!gameLocal.isClient) {
        ...
        // if this is a power ammo weapon ( currently only the bfg ) then make
        // sure we only fire as much power as available in each clip
        if (powerAmmo) {
        ...
        owner->inventory.UseAmmo(ammoType,
                                 (powerAmmo) ? dmgPower : ammoRequired);
        if (clipSize && ammoRequired) {
    ...
    // which the gun material shaders can reference for single shot barrel
    // glows, etc
    renderEntity.shaderParms[SHADERPARM_DIVERSITY] =
        gameLocal.random.CRandomFloat();
    renderEntity.shaderParms[SHADERPARM_TIMEOFFSET] =
        -MS2SEC(gameLocal.realClientTime);

    if (worldModel.GetEntity()) {
        worldModel.GetEntity()->SetShaderParm(
            SHADERPARM_DIVERSITY,
            renderEntity.shaderParms[SHADERPARM_DIVERSITY]);
        worldModel.GetEntity()->SetShaderParm(
            SHADERPARM_TIMEOFFSET,
            renderEntity.shaderParms[SHADERPARM_TIMEOFFSET]);
    }

    // calculate the muzzle position
    if (barrelJointView != INVALID_JOINT &&
        projectileDict.GetBool("launchFromBarrel")) {
    ...
    // add some to the kick time, incrementally moving repeat firing weapons
    // back
    ...
    if (gameLocal.isClient) {
        ...
        if (projectileDict.GetBool("net_instanthit")) {
            ...
            for (i = 0; i < num_projectiles; i++) {
                ...
                dir = playerViewAxis[0] +
                      playerViewAxis[2] * (ang * idMath::Sin(spin)) -
                      playerViewAxis[1] * (ang * idMath::Cos(spin));
                dir.Normalize();
                gameLocal.clip.Translation(
                    tr, muzzle_pos, muzzle_pos + dir * 4096.0f, NULL,
                    mat3_identity, MASK_SHOT_RENDERMODEL, owner);
                if (tr.fraction < 1.0f) {
                    idProjectile::ClientPredictionCollide(
                        this, projectileDict, tr, vec3_origin, true);
                }
            }
        }
        ...
        for (i = 0; i < num_projectiles; i++) {
            ...
            projBounds = proj->GetPhysics()->GetBounds().Rotate(
                proj->GetPhysics()->GetAxis());

            // make sure the projectile starts inside the bounding box of the
            // owner
            if (i == 0) {
                muzzle_pos = muzzleOrigin + playerViewAxis[0] * 2.0f;
                // DG: sometimes the assertion in idBounds::operator-(const
                // idBounds&) triggers
                //     (would get bounding box with negative volume)
                //     => check that before doing ownerBounds - projBounds
                //     (equivalent to the check in the assertion)
                idVec3 obDiff = ownerBounds[1] - ownerBounds[0];
                idVec3 pbDiff = projBounds[1] - projBounds[0];
                bool boundsSubLegal = obDiff.x > pbDiff.x &&
                                      obDiff.y > pbDiff.y &&
                                      obDiff.z > pbDiff.z;
                if (boundsSubLegal &&
                    (ownerBounds - projBounds)
                        .RayIntersection(muzzle_pos, playerViewAxis[0],
                                         distance)) {
                ...
                gameLocal.clip.Translation(
                    tr, start, muzzle_pos, proj->GetPhysics()->GetClipModel(),
...
void idWeapon::Event_Melee(void) {
    ...
    if (!meleeDef) {
        gameLocal.Error("No meleeDef on '%s'",
                        weaponDef->dict.GetString("classname"));
    }

    if (!gameLocal.isClient) {
        idVec3 start = playerViewOrigin;
        idVec3 end = start + playerViewAxis[0] *
                                 (meleeDistance *
                                  owner->PowerUpModifier(MELEE_DISTANCE));
        gameLocal.clip.TracePoint(tr, start, end, MASK_SHOT_RENDERMODEL, owner);
        if (tr.fraction < 1.0f) {
            ent = gameLocal.GetTraceEntity(tr);
        } else {
        ...
        if (g_debugWeapon.GetBool()) {
            ...
            if (ent) {
                gameRenderWorld->DebugBounds(
                    colorRed, ent->GetPhysics()->GetBounds(),
                    ent->GetPhysics()->GetOrigin(), 100);
            }
        }
        ...
        if (ent) {
            ...
            idVec3 impulse =
                -push * owner->PowerUpModifier(SPEED) * tr.c.normal;
            ...
            // weapon stealing - do this before damaging so weapons are not
            // dropped twice
            if (gameLocal.isMultiplayer && weaponDef &&
                ...
                (gameLocal.gameType != GAME_TDM ||
                 ...
                 (owner->team != static_cast<idPlayer*>(ent)->team))) {
            ...
            if (ent->fl.takedamage) {
                ...
                ent->Damage(owner, owner, globalKickDir, meleeDefName,
                            owner->PowerUpModifier(MELEE_DAMAGE), tr.c.id);
                hit = true;
            }

            if (weaponDef->dict.GetBool("impact_damage_effect")) {
                if (ent->spawnArgs.GetBool("bleed")) {
                    hitSound = meleeDef->dict.GetString(
                        owner->PowerUpActive(BERSERK) ? "snd_hit_berserk"
                                                      : "snd_hit");

                    ent->AddDamageEffect(tr, impulse,
                                         meleeDef->dict.GetString("classname"));
                    ...
                    hitSound =
                        meleeDef->dict.GetString(va("snd_%s", materialType));
                    if (*hitSound == '\0') {
                    ...
                    if (gameLocal.time > nextStrikeFx) {
                        ...
                        if (decal && *decal) {
                            gameLocal.ProjectDecal(tr.c.point, -tr.c.normal,
                                                   8.0f, true, 6.0, decal);
                        }
...
void idWeapon::Event_EjectBrass(void) {
    ...
    if (!ent || !ent->IsType(idDebris::Type)) {
        gameLocal.Error("'%s' is not an idDebris",
                        weaponDef ? weaponDef->dict.GetString("def_ejectBrass")
                                  : "def_ejectBrass");
    }
    ...
    linear_velocity =
        40 * (playerViewAxis[0] + playerViewAxis[1] + playerViewAxis[2]);
    angular_velocity.Set(10 * gameLocal.random.CRandomFloat(),
                         10 * gameLocal.random.CRandomFloat(),
                         10 * gameLocal.random.CRandomFloat());
