...
#define MAX_SIGNAL_THREADS \
    16  // probably overkill, but idList uses a granularity of 16
...
class idEntity : public idClass {
   public:
    ...
    idLinkList<idEntity>
        spawnNode;  // for being linked into spawnedEntities list
    idLinkList<idEntity>
        activeNode;  // for being linked into activeEntities list

    idLinkList<idEntity>
        snapshotNode;      // for being linked into snapshotEntities list
    int snapshotSequence;  // last snapshot this entity was in
    int snapshotBits;      // number of bits this entity occupied in the last
                           // snapshot

    idStr name;        // name of entity
    idDict spawnArgs;  // key/value pairs used to spawn and initialize entity
    idScriptObject
        scriptObject;  // contains all script defined data for this entity

    int thinkFlags;    // TH_? flags
    int dormantStart;  // time that the entity was first closed off from player
    bool cinematic;    // during cinematics, entity will only think if cinematic
                       // is set
    ...
    idList<idEntityPtr<idEntity> >
        targets;  // when this entity is activated these entities entity are
                  // activated
    ...
    struct entityFlags_s {
        ...
        bool bindOrientated : 1;  // if true both the master orientation is used
                                  // for binding
        bool solidForTeam : 1;  // if true this entity is considered solid when
                                // a physics team mate pushes entities
        ...
        bool hasAwakened : 1;   // before a monster has been awakened the first
                                // time, use full PVS for dormant instead of
                                // area-connected
        bool networkSync : 1;   // if true the entity is synchronized over the
                                // network
    } fl;
   ...
   public:
    ...
    virtual void DormantEnd(
        void);  // called when entity wakes from being dormant
    ...
    virtual idAnimator* GetAnimator(
        void);  // returns animator object used by this entity

    // sound
    virtual bool CanPlayChatterSounds(void) const;
    bool StartSound(const char* soundName,
                    const s_channelType channel,
                    int soundShaderFlags,
                    bool broadcast,
                    int* length);
    bool StartSoundShader(const idSoundShader* shader,
                          const s_channelType channel,
                          int soundShaderFlags,
                          bool broadcast,
                          int* length);
    ...
    void GetWorldVelocities(idVec3& linearVelocity,
    ...
    // use angles to set the axis of the physics object (relative to bindMaster
    // if not NULL)
    ...
    virtual void GetImpactInfo(idEntity* ent,
                               int id,
                               const idVec3& point,
                               impactInfo_t* info);
    // apply an impulse to the physics object, 'ent' is the entity applying the
    // impulse
    virtual void ApplyImpulse(idEntity* ent,
                              int id,
                              const idVec3& point,
                              const idVec3& impulse);
    // add a force to the physics object, 'ent' is the entity adding the force
    virtual void AddForce(idEntity* ent,
                          int id,
                          const idVec3& point,
                          const idVec3& force);
    ...
    virtual void Damage(idEntity* inflictor,
                        idEntity* attacker,
                        const idVec3& dir,
    ...
    virtual void AddDamageEffect(const trace_t& collision,
                                 const idVec3& velocity,
                                 const char* damageDefName);
    // callback function for when another entity received damage from this
    // entity.  damage can be adjusted and returned to the caller.
    virtual void DamageFeedback(idEntity* victim,
                                idEntity* inflictor,
                                int& damage);
    // notifies this entity that it is in pain
    virtual bool Pain(idEntity* inflictor,
                      idEntity* attacker,
                      int damage,
                      const idVec3& dir,
                      int location);
    // notifies this entity that is has been killed
    virtual void Killed(idEntity* inflictor,
                        idEntity* attacker,
                        int damage,
                        const idVec3& dir,
                        int location);
    ...
    void SetSignal(signalNum_t signalnum,
                   idThread* thread,
                   const function_t* function);
    ...
    // misc
    virtual void Teleport(const idVec3& origin,
    ...
    void ServerSendEvent(int eventId,
                         const idBitMsg* msg,
                         bool saveEvent,
   ...
   private:
    ...
    void Event_BindToJoint(idEntity* master,
    ...
    void Event_SetShaderParms(float parm0,
                              float parm1,
                              float parm2,
                              float parm3);
...
class idAnimatedEntity : public idEntity {
   public:
    ...
    bool GetJointWorldTransform(jointHandle_t jointHandle,
                                int currentTime,
                                idVec3& offset,
                                idMat3& axis);
    bool GetJointTransformForAnim(jointHandle_t jointHandle,
                                  int animNum,
                                  int currentTime,
                                  idVec3& offset,
                                  idMat3& axis) const;
    ...
    virtual void AddDamageEffect(const trace_t& collision,
                                 const idVec3& velocity,
                                 const char* damageDefName);
    void AddLocalDamageEffect(jointHandle_t jointNum,
   ...
   private:
    ...
    void Event_SetJointPos(jointHandle_t jointnum,
                           jointModTransform_t transform_type,
                           const idVec3& pos);
    void Event_SetJointAngle(jointHandle_t jointnum,
                             jointModTransform_t transform_type,
                             const idAngles& angles);
